...
namespace Nop.Web.Controllers
{
    public partial class CustomerController : BasePublicController
    {
        ...
        private readonly IOrderService _orderServiceNetSuite;
        private readonly ICustomerAuthorizeNetService _customerAuthorizeNetService;
        private readonly ISettingService _settingService;
        private readonly IPluginManager<ICreditCardVault> _pluginManager;
        ...
        public CustomerController(AddressSettings addressSettings,
            ...
            ICompanyService companyService, ICustomerAuthorizeNetService customerAuthorizeNetService, ISettingService settingService, IPluginManager<ICreditCardVault> pluginManager,
                    ...
                    case CustomerLoginResults.Successful:
                        {
                            ...
                            {
                                _companyService.SetDefaulCompanyCustomer(Convert.ToInt32(Request.Form["Company"]), customer.Id);
                            }
                            ...
                            if (string.IsNullOrEmpty(returnUrl) || !Url.IsLocalUrl(returnUrl) || returnUrl.Contains("validateregister") || returnUrl.Contains("login"))
        ...
        //available even when navigation is not allowed
        [CheckAccessPublicStore(true)]
        public ActionResult CheckUserCompanyAvailability(string username)
        {
            var company = new List<Company>();
            var customer = _customerService.GetCustomerByUsername(username);
            if (customer != null)
            {
                if (customer.NetsuitId != 0)
                {
                    var customerPasswordExist = _customerService.GetCurrentPassword(customer.Id);
                    if (customerPasswordExist != null)
                    {
                        var companyList = _companyService.GetAllCompanyCustomerMappings(customer.Id).ToList();
                        var ContainSetDefaul = companyList.Where(r => r.DefaultCompany).FirstOrDefault();

                        if (companyList.Count > 0 && ContainSetDefaul == null)
                        {
        ...
        //available even when navigation is not allowed
        [CheckAccessPublicStore(true)]
        public virtual IActionResult Register(string email)
        {
            ...
            model.Email = email;
            model.DisabledEmail = !string.IsNullOrEmpty(email);
        ...
        //available even when navigation is not allowed
        [CheckAccessPublicStore(true)]
        public virtual IActionResult Register(RegisterModel model, string email, string returnUrl, bool captchaValid, IFormCollection form)
        {
            //check whether registration is allowed
            if (_customerSettings.UserRegistrationType == UserRegistrationType.Disabled)
                return RedirectToRoute("RegisterResult", new { resultId = (int)UserRegistrationType.Disabled });
            ...
            if (ModelState.IsValid)
            {
                ...
                customer.Companies.Add(new Company { CompanyName = model.Company });
        ...
        //available even when navigation is not allowed
        [CheckAccessPublicStore(true)]
        public virtual IActionResult ValidateRegister(bool accountAlreadyRegister = false)
        {
            //check whether registration is allowed
            if (_customerSettings.UserRegistrationType == UserRegistrationType.Disabled)
                return RedirectToRoute("RegisterResult", new { resultId = (int)UserRegistrationType.Disabled });
            ...
            var model = new RegisterModel();
            model = _customerModelFactory.PrepareRegisterModel(model, false, setDefaultValues: true);
            model.AccountAlreadyRegister = accountAlreadyRegister;
            model.AccountAlreadyLogin = !string.IsNullOrEmpty(_workContext.CurrentCustomer.Username);
        ...
        //available even when navigation is not allowed
        [CheckAccessPublicStore(true)]
        public virtual IActionResult ValidateRegister(RegisterModel model, string returnUrl, bool captchaValid, IFormCollection form)
        {
            //check whether registration is allowed
            if (_customerSettings.UserRegistrationType == UserRegistrationType.Disabled)
                return RedirectToRoute("RegisterResult", new { resultId = (int)UserRegistrationType.Disabled });
            ...
            if (ModelState.IsValid)
            {
                ...
                if (!string.IsNullOrEmpty(model.Email))
                {
                    ...
                    var customerExist = _customerRegistrationService.ValidateEmailExist(model.Email);
                    if (customerExist != null)
                    {
                        ...
                        if (customerExist.NetsuitId != 0)
                        {
                            ...
                            if (customerPasswordExist != null)
                            {
                                // Show message email is register link login
                                return RedirectToRoute("ValidateRegister", new { accountAlreadyRegister = true });
                            }
                            else
                            {
                                // notifications Send Email to Customer
                                _workflowMessageService.SendCustomerCreatePasswordMessage(customerExist, _localizationSettings.DefaultAdminLanguageId);

                                //raise event       
                                _eventPublisher.Publish(new CustomerRegisteredEvent(customer));
                            }

                            return RedirectToRoute("RegisterResult", new { resultId = (int)UserRegistrationType.SendMessagePasswordCreate });
                        }
                    ...
                    {
                        // Go to web guest register
                        return RedirectToRoute("Register", new { email = model.Email });
                    }
                }
        ...
        //available even when navigation is not allowed
        [CheckAccessPublicStore(true)]
        public virtual IActionResult PasswordCreateConfirm(string token, string email)
        {
            var customer = _customerService.GetCustomerByGuidEmail(new Guid(token), email);
            if (customer == null)
            {
                return View(new PasswordRecoveryConfirmModel
                {
                    DisablePasswordChanging = true,
                    Result = _localizationService.GetResource("Account.PasswordRecovery.WrongToken")
                });
            }
        ...
        [HttpPost, ActionName("PasswordCreateConfirm")]
        [PublicAntiForgery]
        [FormValueRequired("set-password")]
        //available even when navigation is not allowed
        [CheckAccessPublicStore(true)]
        public virtual IActionResult PasswordCreateConfirmPOST(string token, string email, PasswordRecoveryConfirmModel model)
        {
            var customer = _customerService.GetCustomerByGuidEmail(new Guid(token), email);
            if (customer == null)
                return RedirectToRoute("Homepage");
            ...
            var customerToken = _customerService.GetCustomerByGuidEmail(new Guid(token), email);
            ...
            if (ModelState.IsValid)
            {
                ...
                if (response.Success)
                {
                    ...
                    _genericAttributeService.SaveAttribute(customer, NopCustomerDefaults.PasswordCreateTokenAttribute, "");

                    model.DisablePasswordChanging = true;
                    model.Result = _localizationService.GetResource("Account.PasswordRecovery.PasswordHasBeenCreated");
                    ...
                    try
                    {
                        var GetPendingDataToSync = _pendingDataToSyncService.GetPendingDataToSync(Convert.ToInt32(customer.NetsuitId));
                        ...
                        else
                        {
                            ...
                            items.IdItem = Convert.ToInt32(customer.NetsuitId);
                            items.Synchronized = false;
                            items.Type =(int)ImporterIdentifierType.ContactCustomer;
                    ...
                    {
                        _logger.Warning("Update Contact to PendingDataToSync: " + customer.NetsuitId, ex);
                    }
        ...
        public virtual IActionResult Info(CustomerInfoModel model, IFormCollection form)
        {
            ...
            try
            {
                if (!string.IsNullOrEmpty(form["nextstep"]))
                {
                    if (form["group1[]"] == "on")
                    {
                        SaveCreditCartInfo(model, form);
                    }
                }
                else
                {
        ...
        [PublicAntiForgery]
        [HttpsRequirement(SslRequirement.Yes)]
        public  string SaveCreditCartInfo(CustomerInfoModel model, IFormCollection form)
        {
            var message = "";
            var messageStatus = "";
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(form["nextstep"]))
                {
                    if (form["group1[]"] == "on")
                    {
                        string[] CardholderName = form["CardholderName"].ToString().Split(',');
                        string[] CardNumber     = form["CardNumber"].ToString().Split(',');
                        string[] CardCode       = form["CardCode"].ToString().Split(',');
                        string[] ExpireMonth    = form["ExpireMonth"].ToString().Split(',');
                        string[] ExpireYear     = form["ExpireYear"].ToString().Split(',');

                        model.CustomerPaymentInfoModel.CardholderName = CardholderName[1];
                        model.CustomerPaymentInfoModel.CardNumber = CardNumber[1];
                        model.CustomerPaymentInfoModel.CardCode = CardCode[1];
                        model.CustomerPaymentInfoModel.ExpireMonth = ExpireMonth[1];
                        model.CustomerPaymentInfoModel.ExpireYear = ExpireYear[1];

                       
                        var creditCardVault = _pluginManager.LoadPluginBySystemName("Misc.CreditCardVaultAuthorizeNet");

                        if (creditCardVault != null && creditCardVault.PluginDescriptor.Installed)
                        {
                            ...
                            else
                            {
                                ...
                                address.StateProvinceId = Convert.ToInt32(AddressIdSelected[3]);
                                address.ZipPostalCode = AddressIdSelected[2];
                                address.CountryId = Convert.ToInt32(AddressIdSelected[4]);
                            }
                                var result = creditCardVault.SaveCreditCard(_workContext.CurrentCustomer, new ProcessPaymentRequest
                            {
                                CreditCardName = model.CustomerPaymentInfoModel.CardholderName,
                                CreditCardNumber = model.CustomerPaymentInfoModel.CardNumber,
                                CreditCardExpireMonth = model.CustomerPaymentInfoModel.ExpireMonth,
                                CreditCardExpireYear = Convert.ToInt32(model.CustomerPaymentInfoModel.ExpireYear),
                                CreditCardCvv2 = model.CustomerPaymentInfoModel.CardCode,
                                NewAddress= address
                                });
                        ...
                        model.CustomerPaymentInfoModel.ResponsePayment = message;
                        model.CustomerPaymentInfoModel.messageStatus = messageStatus;

                    }
                }
                else
                {
                    if (form["AddCreditCard"] == "on")
                    {
                        string[] CardholderName = form["CardholderName"].ToString().Split(',');
                        string[] CardNumber = form["CardNumber"].ToString().Split(',');
                        string[] CardCode = form["CardCode"].ToString().Split(',');
                        ...
                        model.CustomerPaymentInfoModel.CardholderName = CardholderName[0];
                        model.CustomerPaymentInfoModel.CardNumber = CardNumber[0];
                        model.CustomerPaymentInfoModel.CardCode = CardCode[0];
                        model.CustomerPaymentInfoModel.ExpireMonth = ExpireMonth[0];
                        model.CustomerPaymentInfoModel.ExpireYear = ExpireYear[0];
                        ...
                        if (creditCardVault != null && creditCardVault.PluginDescriptor.Installed)
                        {
                            ...
                            if (NewAddress == "on" || NewAddressMobile == "on")
                            {
                                var state = _stateProvinceService.GetStateProvinceById(Convert.ToInt32(form["StateProvinceId"]));
                                var country = _countryService.GetCountryById(Convert.ToInt32(form["CountryId"]));
                                ;
                                address.Address1 = form["Address1"].ToString();
                                address.City = form["City"].ToString();
                                address.StateProvinceId = Convert.ToInt32(form["StateProvinceId"]);
                                ...
                                address.LastName = form["LastName"].ToString();
                                address.Email = form["Email"].ToString();
                                address.PhoneNumber = form["PhoneNumber"].ToString();
                            }
                            else
                            {
                                var AddressIdSelected = form["AddressIdSelected"].ToString().Split(',');
                                ...
                                address.CountryId = Convert.ToInt32(AddressIdSelected[4]);
                                address.Id = Convert.ToInt32(AddressIdSelected[5]);
                            }
                            var result = creditCardVault.SaveCreditCard(_workContext.CurrentCustomer, new ProcessPaymentRequest
                            {
                                CreditCardName = model.CustomerPaymentInfoModel.CardholderName,
            ...
            {
                ModelState.AddModelError("", exc.Message);
                messageStatus = "Card or address information is not valid. Please try again or contact support.";
            }
        ...
        [PublicAntiForgery]
        [HttpsRequirement(SslRequirement.Yes)]
        public virtual IActionResult AddressCompanyDelete(int addressId)
        {
            if (!_workContext.CurrentCustomer.IsRegistered())
                return Challenge();

            //find address (ensure that it belongs to the current customer)
            var address = _companyService.GetCompanyByAddressId(addressId);
            if (address != null)
            {
                _companyService.DeleteCompanyAddressMapping(address);
            }
        ...
        public virtual IActionResult AddressAdd(CustomerAddressEditModel model, IFormCollection form)
        {
            ...
            if (!string.IsNullOrEmpty(model.Address.ZipPostalCode))
            {
                ...
                ValidAdd.City = model.Address.City;

                if (model.Address.CountryId.HasValue)
                    ValidAdd.Country = _countryService.GetCountryById(model.Address.CountryId.Value);
                if (model.Address.StateProvinceId.HasValue)
                    ValidAdd.StateProvince = _stateProvinceService.GetStateProvinceById(model.Address.StateProvinceId.Value);

                ValidAdd.Address1 = model.Address.Address1;
                ValidAdd.ZipPostalCode = model.Address.ZipPostalCode;
                ...
                if (!validAddress.IsValid)
                {
                    ...
                    //if (!_zipCodeService.IsValidCode((int)model.Address.CountryId, (int)model.Address.StateProvinceId, model.Address.ZipPostalCode))
                    // {
                    model.Address.ValidZipCode = true;
                    ModelState.AddModelError(model.Address.ZipPostalCode, "Invalid Zip Code"); //throw new Exception("Zip Code invalid");
				...
				{
                    model.Address.ValidZipCode = false;

                    if (model.Address.PopupOpen)
                    {
            ...
            if (model.Address.PhoneNumber==null)
            {
                ...
                model.Address.ValidPhoneNumber = true;
                ModelState.AddModelError("ErrorAddressPhoneNumber", "Invalid PhoneNumber"); //throw new Exception("Zip Code invalid");
            }
            ...
            if (ModelState.IsValid)
            {
                ...
                address.Residential = model.Address.Residential;

                //customer.Addresses.Add(address);
                customer.CustomerAddressMappings.Add(new CustomerAddressMapping { Address = address });
                _customerService.UpdateCustomer(customer);

                if (model.Address.IsBilling)
                {
                    customer.BillingAddressId = address.Id;
                    customer.BillingAddress = address;
                }

                if (model.Address.IsShipping)
                {
        ...
        public virtual IActionResult AddressEdit(CustomerAddressEditModel model, int addressId, IFormCollection form)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (_workContext.CurrentCustomer.Companies.Any())
                {
                    if (_workContext.WorkingCompany != null)
                    {
                        var AddressCompany = _workContext.WorkingCompany.CompanyAddressMappings.FirstOrDefault(a => a.AddressId == addressId);
                        if (model.Address.IsBilling)
                        {
                           var addressBilling= _workContext.WorkingCompany.CompanyAddressMappings.Where(a => a.IsBilling);

                            foreach (var item in addressBilling)
                            {
                                item.IsBilling = false;
                                _companyService.UpdateCompanyAddressMapping(item);
                            }

                            AddressCompany.IsBilling =true;
                            AddressCompany.Address = address;
                            AddressCompany.AddressId = address.Id;
                        }
                        else
                        {
                            AddressCompany.IsBilling = false;
                        }

                        if (model.Address.IsShipping)
                        {
                            var addressBilling = _workContext.WorkingCompany.CompanyAddressMappings.Where(a => a.IsShipping);
                            ...
                            AddressCompany.IsShipping = true;
                            AddressCompany.Address = address;
                            AddressCompany.AddressId = address.Id;
                        }
                        else
                        {
                            AddressCompany.IsShipping = false;
                        }
                        
                        _companyService.UpdateCompanyAddressMapping(AddressCompany);
                    }
                }
        ...
        [PublicAntiForgery]
        [HttpsRequirement(SslRequirement.Yes)]
        public virtual IActionResult InactiveInfo(int cp, int ccp)
        {
            if (!_workContext.CurrentCustomer.IsRegistered())
                return Challenge();

            var model = new DeleteCustomerPaymentProfileRequestModel
            {
                customerPaymentProfileId = ccp.ToString(),
                customerProfileId = cp.ToString()
            };

            _customerAuthorizeNetService.DeleteCustomerPaymentProfile(model);


             _customerAuthorizeNetService.DeletePaymentById(cp, ccp);
        ...
        [PublicAntiForgery]
        [HttpsRequirement(SslRequirement.Yes)]
        public virtual IActionResult SavePaymentInfo(CheckoutPaymentInfoModel model)
        {
            //if (!_workContext.CurrentCustomer.IsRegistered())
            //    return Challenge();
