...
namespace Nop.Web.Controllers
{
    public partial class ShoppingCartController : BasePublicController
    {
        ...
        protected virtual IActionResult GetProductToCartDetails(List<string> addToCartWarnings, ShoppingCartType cartType,
            Product product, int totalProducts)
        {
        ...
        [HttpPost]
        public bool ValidItemsCollection(IFormCollection form)
        {
            var brandedItemValue = form["brandedItemValue"];
            var productId = form["value"];



           // string[] parts = brandedItemValue.Split(',');
            ...
            int quantity, bool forceredirection = false)
        {
            ...
            {
                quantity = product.IncrementQuantity;
            }
                ...
                default:
                    {
                        ...
                        return Json(new
                        {
                            ...
                            updatetopcartsectionhtml,
                            updateflyoutcartsectionhtml,
                            totalProducts = shoppingCarts.Sum(item => item.Quantity)
                        });
                    }
            }
        }
        ...
        public virtual IActionResult AddProductToCart_Details(int productId, int shoppingCartTypeId, IFormCollection form)
        {
            ...
            var totalProducts = cart.Sum(item => item.Quantity);
            ...
            //return result
            return GetProductToCartDetails(addToCartWarnings, cartType, product, totalProducts + quantity);
        }
            ...
            bool loadPicture, IFormCollection form)
        {
            ...
            if (_permissionService.Authorize(StandardPermissionProvider.DisplayPrices) && !product.CustomerEntersPrice)
            {
                ...
                var finalPriceWithDiscountBase = _taxService.GetProductPrice(product, finalPrice, out decimal _,false, decimal.Zero);
        ...
        public virtual IActionResult Cart()
        {
            ...
            model = _shoppingCartModelFactory.PrepareShoppingCartModel(model, cart, prepareAndDisplayRelatedProducts: true);
        ...
        public JsonResult UpdateCartItem(IFormCollection form)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.EnableShoppingCart))
                return Json("_permissionService Denied");
            ...
            {
                //try to get a new quantity for the item, set 0 for items to remove
                NewQuantity = itemIdsToRemove.Contains(item.ProductId) ? 0 : int.TryParse(form[$"itemquantity{item.Id}"], out int quantity) ? quantity : item.Quantity,
                Item = item
            }).Where(item => item.NewQuantity != item.Item.Quantity);
            ...
            var warnings = orderedCart.Select(cartItem => new
            {
                ItemId = cartItem.Item.ProductId,
                Warnings = _shoppingCartService.UpdateShoppingCartItem(_workContext.CurrentCustomer,
                    cartItem.Item.Id, cartItem.Item.AttributesXml, cartItem.Item.CustomerEnteredPrice,
                    cartItem.Item.RentalStartDateUtc, cartItem.Item.RentalEndDateUtc, cartItem.NewQuantity, true)
            }).ToList();
            ...
            var total = _shoppingCartModelFactory.PrepareOrderTotalsModel(cart, false);
            model.SubTotal = total.SubTotal;
            model.OrderTotal = total.OrderTotal;
            model.OrderTax = total.Tax;
        ...
        public string ClearShoppingCart(IFormCollection form)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.EnableShoppingCart))
                return "0";
        ...
        [HttpPost, ActionName("WishlistItem")]
        [FormValueRequired("updatecart")]
        public JsonResult UpdateWishlistItem(IFormCollection form)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.EnableShoppingCart))
                return Json("_permissionService Denied");
        ...
        public virtual IActionResult AddItemsToCartFromWishlist(Guid? customerGuid, IFormCollection form)
        {
            ...
            var allIdsToAdd = form.ContainsKey("itemquantity")
                ? form["itemquantity"].ToString().Split(new[] { ',' }, StringSplitOptions.RemoveEmptyEntries).Select(int.Parse).ToList()
                : new List<int>();
            foreach (var sci in pageCart)
            {
                ...
                //activity log
                _customerActivityService.InsertActivity("PublicStore.AddToShoppingCart",
                    string.Format(_localizationService.GetResource("ActivityLog.PublicStore.AddToShoppingCart"), sci.Product.Name), sci.Product);
