...
namespace Nop.Web.Areas.Admin.Controllers
{
    public partial class ShippingController : BaseAdminController
    {
        ...
        public ShippingController(IAddressService addressService,
            ...
            ShippingSettings shippingSettings, IWarehouseLocationNNService warehouseLocationNNService)
        {
        ...
        #region WarehousesLocationNN

        public virtual IActionResult WarehousesLocationNN()
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageShippingSettings))
                return AccessDeniedView();

            //prepare model
            var model = _shippingModelFactory.PrepareWarehouseLocationNNSearchModel(new WarehouseLocationNNSearchModel());

            return View(model);
        }

        [HttpPost]
        public virtual IActionResult WarehousesLocationNN(WarehouseLocationNNSearchModel searchModel)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageShippingSettings))
                return AccessDeniedDataTablesJson();

            //prepare model
            var model = _shippingModelFactory.PrepareNNWarehouseListModel(searchModel);

            return Json(model);
        }

        [HttpGet]
        public JsonResult SearchWarehouses(string query)
        {
        ...
        [HttpPost]
        public JsonResult SaveSelectedWarehouse(int warehouseId, int StateProvidenceId)
        {
            var WarehouseLocationNN = _warehouseLocationNNService.GetWarehouseLocationId(StateProvidenceId);

            if(WarehouseLocationNN!=null)
			{
                WarehouseLocationNN.WarehouseId = warehouseId;

                _warehouseLocationNNService.UpdateWarehouseLocation(WarehouseLocationNN);
            }
