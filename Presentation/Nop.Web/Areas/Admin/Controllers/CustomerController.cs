...
namespace Nop.Web.Areas.Admin.Controllers
{
    public partial class CustomerController : BaseAdminController
    {
        ...
        public CustomerController(CustomerSettings customerSettings,
            ...
            TaxSettings taxSettings, ICompanyService companyService)
        {
        ...
        public virtual IActionResult CompanyList(CompanyCustomerSearchModel searchModel)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCustomers))
                return AccessDeniedDataTablesJson();

            //try to get a customer with the specified id
            var customer = _companyService.GetAllCompanyCustomerMappings(searchModel.CustomerId)
                ?? throw new ArgumentException("No customer found with the specified id");

            //prepare model
            var model = _customerModelFactory.PrepareCustomerCompanyListModel(searchModel, customer);
