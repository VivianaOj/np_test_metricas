...
namespace Nop.Web.Areas.Admin.Factories
{
    ...
    public partial class OrderModelFactory : IOrderModelFactory
    {
        ...
        public OrderModelFactory(AddressSettings addressSettings,
            ...
            TaxSettings taxSettings, IBoxesGeneratorServices boxPackingService, ICompanyService companyServices, IInvoiceService invoiceService)
        {
        ...
        protected virtual void PrepareOrderModelTotals(OrderModel model, Order order)
        {
            ...
            model.OrderSubtotalInclTax = order.OrderSubtotalInclTax.ToString("G15");
            ...
            model.OrderSubtotalExclTax = order.OrderSubtotalExclTax.ToString("G15");
            ...
            if (order.Source?.Contains("REST")==true)
                model.Source = "WEB";
            else
                model.Source = "NS";
            ...
            model.Profit = profit.ToString("G15");
        }
        ...
        protected virtual void PrepareOrderModelPaymentInfo(OrderModel model, Order order)
        {
            ...
            {
                if (order.MaskedCreditCardNumber != "0")
                {
                ...
                {
                    model.CardNumber = order.MaskedCreditCardNumber;
                }
            ...
            if (!order.PickupInStore)
            {
                ...
                if (shipments.Count > 0)
                {
                    ...
                    if (shipmentTracker != null)
                    {
                        ...
                        {
                            order.ShippingStatus = GetShippingEvent(shipmentStatusEventsEvent.EventName);
                            order.ShippingStatusId = (int)order.ShippingStatus;
                        }
                    }
                }
                ...
                if (attribute.ShouldHaveValues())
                {
                    ...
                    foreach (var attributeValue in attributeValues)
                    {
                        ...
                        var priceAdjustment = _taxService.GetProductPrice(product,
                            _priceCalculationService.GetProductAttributeValuePriceAdjustment(attributeValue, order.Customer), out _, false, decimal.Zero);
        ...
        public virtual OrderSearchModel PrepareOrderSearchModel(OrderSearchModel searchModel)
        {
            ...
            //prepare available companies
            _baseAdminModelFactory.PrepareCompanyList(searchModel.AvailableCompanyList);
            if (searchModel.AvailableCompanyList.Any())
            {
                if (searchModel.CompanyListId?.Any() ?? false)
                {
                    var ids = searchModel.CompanyListId.Select(id => id.ToString());
                    searchModel.AvailableCompanyList.Where(statusItem => ids.Contains(statusItem.Value)).ToList()
                        .ForEach(statusItem => statusItem.Selected = true);
                }
                else
                    searchModel.AvailableCompanyList.FirstOrDefault().Selected = true;
            }

            //prepare available userRole
            _baseAdminModelFactory.PrepareUserRoleList(searchModel.AvailableUserRoleList);
            if (searchModel.AvailableUserRoleList.Any())
            {
                if (searchModel.UserRoleId?.Any() ?? false)
                {
                    var ids = searchModel.UserRoleId.Select(id => id.ToString());
                    searchModel.AvailableUserRoleList.Where(statusItem => ids.Contains(statusItem.Value)).ToList()
                        .ForEach(statusItem => statusItem.Selected = true);
                }
                else
                    searchModel.AvailableUserRoleList.FirstOrDefault().Selected = true;
            }


            if (!searchModel.AvailableSource.Any())
            {
                searchModel.AvailableSource.Insert(0, new SelectListItem { Text = _localizationService.GetResource("Admin.Common.Web"), Value = "Web" });
                searchModel.AvailableSource.Insert(1, new SelectListItem { Text = _localizationService.GetResource("Admin.Common.NS"), Value = "NS" });
                searchModel.AvailableSource.Insert(2, new SelectListItem { Text = _localizationService.GetResource("Admin.Common.All"), Value = "All" });
        ...
        public virtual OrderListModel PrepareOrderListModel(OrderSearchModel searchModel)
        {
            ...
            var CompanyIds = (searchModel.CompanyListId?.Contains(0) ?? true) ? null : searchModel.CompanyListId.ToList();
            var UserRoleIds = (searchModel.UserRoleId?.Contains(0) ?? true) ? null : searchModel.UserRoleId.ToList();
            ...
            var orders = _orderService.SearchOrders(storeId: searchModel.StoreId,
                ...
                SourceName: searchModel.SourceName,
            ...
            var model = new OrderListModel().PrepareToGrid(searchModel, orders, () =>
            {
                ...
                {
                    //fill in model values from the entity
                    var CustomOrderNumber = order.CustomOrderNumber;
                    ...
                    if (invoice == null)
                    {
                        ...
                        {
                            CompanyId = _companyServices.GetCompanyById(Convert.ToInt32(order.CompanyId));
                            if(CompanyId!=null)
                    ...
                    var orderModel = new OrderModel
                    {
                        ...
                        CustomerEmail = order.Customer.IsRegistered() ? order.Customer.Email : order.BillingAddress.Email,
                    ...
                    var CompanyName = CompanyId?.CompanyName;
                    var CompanyNetsuiteId = CompanyId?.NetsuiteId;
                    if (order.CompanyId == 6633)
					{
                        CompanyName = "Web Store Customer";
                        CompanyNetsuiteId = order.CompanyId.ToString();

                    }

                    if (order.Source?.Contains("REST")==true)
                        orderModel.Source = "WEB";
                    else
                        orderModel.Source = "NS";

                    orderModel.CompanyName =   CompanyName +" ("+ CompanyNetsuiteId+")";
                    orderModel.UserRolesNames =  string.Join(", ", order.Customer.CustomerRoles.Select(role => role.Name));

                    //orderModel.OrderTotal = _priceFormatter.FormatPrice(order.OrderTotal, true, false);
                    orderModel.OrderTotal = order.OrderTotal.ToString("G15");
                    return orderModel;
                });
            });
        ...
        public virtual OrderModel PrepareOrderModel(OrderModel model, Order order, bool excludeProperties = false)
        {
            ...
            if (order != null)
            {
                ...
                #region Start NN New changes invoice
                var invoiceNum = "";
                var invoice = _invoiceService.GetInvoicesByCustomerOrderId(order.CustomerId, order.Id);
                ...
                #region Start NN New changes show admin fields 
                var role = order.Customer.GetCustomerRoleIds().ToString();
                model.CustomerRole = order.Customer.IsRegistered() ? string.Join(", ", order.Customer.CustomerRoles.Select(x => x.Name)):"";
                model.CompanyName = order.Customer.IsRegistered() ?  CompanyId?.CompanyName : "";
                model.TransactionLocation = _shippingService.GetLocation(order);
                model.NNdeliveryRouteLocation = order.ShippingNNDeliverySend;
                model.CustomOrderNumber = order.tranId;
                ...
                PrepareOrderModelShippingPackingInfo(model, order);
            }
            ...
            //N&N saveaddresstomyaccount
            #region N&N Saveaddresstomyaccount
            var IsCommercial = _genericAttributeService.GetAttribute<ShippingOption>(order.Customer,
                              NopCustomerDefaults.SelectedShippingOptionAttributeIsCommercial);
            ...
            if (saveaddresstomyaccount != null)
            {
                if (saveaddresstomyaccount)
                    model.BillingAddress.SavedAddressRequest = true;
                else
                    model.BillingAddress.SavedAddressRequest = false;
            }


            if (saveShippingaddresstomyaccount != null)
            {
                if (saveShippingaddresstomyaccount)
                    model.ShippingAddress.SavedAddressRequest = true;
                else
                    model.ShippingAddress.SavedAddressRequest = false;
            }

            if (IsCommercial != null)
            {
                if (IsCommercial.IsCommercial)
                    model.BillingAddress.Residential = false;
                else
                    model.BillingAddress.Residential = true;
            }
            model.BillingAddress.ApprovedNNDelivery = "";
            if (CompanyId?.Id != 0)
			{
                var companyAddress = _companyServices.GetAllCompanyAddressMappingsById(Convert.ToInt32(CompanyId.Id));
				if (companyAddress != null)
				{
                    var addressNN = companyAddress.Where(r => r.AddressId == model.BillingAddress.Id).FirstOrDefault();

                    if (addressNN != null)
                        model.BillingAddress.ApprovedNNDelivery = addressNN.DeliveryRouteName;
                }
            }
        ...
        private void PrepareOrderModelShippingPackingInfo(OrderModel model, Order order)
        {
            //try to get Packing with the specified id
            var BoxGeneratorList = _boxPackingService.GetBSItemPackedBoxList(order.Id);
            ...
            foreach (var item in BoxGeneratorList)
            {
                ...
                {
                    if (BoxGenerator.Container != null)
                        if (BoxGenerator.Container?.ID != 0)
                            BoxInfo = _boxPackingService.GetBoxById(BoxGenerator.Container.ID);


                    var BoxContentWeight = BoxGenerator.PercentItemWeightPacked + BoxInfo.WeigthBox;
                    ...
                    var Packing = _boxPackingService.GetItemPackedBoxId(item.Id);

                    var BoxGeneratorModel = new BoxGeneratorModel
                    {
                        BoxName = BoxInfo?.Name,
                        BoxSize = BoxInfo?.Height + " (Height) x " + BoxInfo?.Width + " (Width) x " + BoxInfo?.Length + " (Length) ",
                        BoxTotalWeight = Math.Round(BoxGenerator.PercentItemWeightPacked).ToString(),
                        BoxContentWeight = Math.Round(BoxContentWeight).ToString(),
                        Active = BoxInfo.Active,
                        VolumenBox = BoxGenerator.TotalVolumePacked,
                    ...
                    model.BoxGenerator.Add(BoxGeneratorModel);
                }
            }
        }
        ...
        public virtual AddProductToOrderModel PrepareAddProductToOrderModel(AddProductToOrderModel model, Order order, Product product)
        {
            ...
            //Start N&N Changes
            var presetPriceInclTax = _taxService.GetProductPrice(product, presetPrice, true, order.Customer, out _, out _, false, decimal.Zero);
            var presetPriceExclTax = _taxService.GetProductPrice(product, presetPrice, false, order.Customer, out _, out _, false, decimal.Zero);
            //End N&N Changes
        ...
        private ShippingStatus GetShippingEvent(string shippingEvent)
        {
            ...
            var stringStatus = trackEvent.ToTitleCase(shippingStatus);

            var shippingEnum = ShippingStatus.NotYetShipped;
            Enum.TryParse(stringStatus, out shippingEnum);
            return shippingEnum;
        }
