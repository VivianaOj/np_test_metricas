...
namespace Nop.Web.Areas.Admin.Factories
{
    ...
    public partial class CustomerModelFactory : ICustomerModelFactory
    {
        ...
        /// <param name="customer">Customer</param>
        /// <returns>Customer order search model</returns>
        protected virtual CompanySearchModel PrepareCustomerCompanySearchModel(CompanySearchModel searchModel, Customer customer)
        {
            if (searchModel == null)
                throw new ArgumentNullException(nameof(searchModel));
        ...
        public virtual CustomerListModel PrepareCustomerListModel(CustomerSearchModel searchModel)
        {
            ...
            var model = new CustomerListModel().PrepareToGrid(searchModel, customers, () =>
            {
                return customers.Select(customer =>
                {
                    ...
                    {
                        if (customer.CompanyCustomerMappings.Count() > 1)
                            customerModel.Company = customer.CompanyCustomerMappings.Where(r => r.DefaultCompany).FirstOrDefault()?.Company.CompanyName;
                        else
                            customerModel.Company = customer.CompanyCustomerMappings.FirstOrDefault()?.Company.CompanyName;
        ...
        public virtual CustomerModel PrepareCustomerModel(CustomerModel model, Customer customer, bool excludeProperties = false)
        {
            if (customer != null)
            {
                ...
                if (!excludeProperties)
                {
					...
					{
                        if(customer.CompanyCustomerMappings.Count()>1)
                            model.Company = customer.CompanyCustomerMappings.Where(r => r.DefaultCompany).FirstOrDefault()?.Company.CompanyName;
                        else
                            model.Company = customer.CompanyCustomerMappings.FirstOrDefault()?.Company.CompanyName;
                ...
                PrepareCustomerCompanySearchModel(model.CompanyModelSearchModel, customer);
            }
            else
            {
                //whether to fill in some of properties
        ...
        public virtual CustomerAddressListModel PrepareCustomerAddressListModel(CustomerAddressSearchModel searchModel, Customer customer)
        {
            ...
            var addresses = customer.Addresses.Where(r=>!r.Active)
            ...
            var model = new CustomerAddressListModel().PrepareToGrid(searchModel, addresses, () =>
            {
                return addresses.Select(address =>
                {
                    ...
                    addressModel.ApprovedNNDelivery = "";
                    if (customer.Companies.Any())
                    {
                        var companyAddress = customer.Companies.SelectMany(r => r.CompanyAddressMappings).FirstOrDefault(x => x.AddressId == address.Id);
                        if (companyAddress != null)
                        {
                            if (companyAddress != null)
                                addressModel.ApprovedNNDelivery = companyAddress.DeliveryRouteName;
                        }
                    }
        ...
        public virtual CompanyCustomerListModel PrepareCustomerCompanyListModel(CompanyCustomerSearchModel searchModel, IList<CompanyCustomerMapping> customer)
        {
            if (searchModel == null)
                throw new ArgumentNullException(nameof(searchModel));
            ...
            //var Company = customer.CompanyCustomerMappings.Where(item => item.CompanyId == searchModel.CompanyId).ToList().ToPagedList(searchModel);

            var pageList = customer.Select(item =>
            {
                //fill in model values from the entity
                var CompModel = new CompanyCustomerModel();

                CompModel.Parent = Convert.ToInt32(item.Company.NetsuiteId);
                CompModel.CompanyName = item.Company.CompanyName ;
                CompModel.Id = item.CompanyId;
                 return CompModel;

            }).ToList().ToPagedList(searchModel);


            var model = new CompanyCustomerListModel().PrepareToGrid(searchModel, pageList, () => pageList);
            ...
            Customer customer)
        {
            ...
            var pageList = shoppingCart.Select(item =>
            {
                ...
                shoppingCartItemModel.UnitPrice = _priceFormatter.FormatPrice(
                    _taxService.GetProductPrice(item.Product, _priceCalculationService.GetUnitPrice(item), out var _,false, decimal.Zero));
                shoppingCartItemModel.Total = _priceFormatter.FormatPrice(
                    _taxService.GetProductPrice(item.Product, _priceCalculationService.GetSubTotal(item), out _, false, decimal.Zero));
                //convert dates to the user time
