...
namespace Nop.Web.Areas.Admin.Factories
{
    ...
    public partial class ProductModelFactory : IProductModelFactory
    {
        ...
        public ProductModelFactory(CatalogSettings catalogSettings,
            ...
            IInvoiceService invoiceService, ICompanyService companyServices, IItemCollectionServices itemCollection)
        {
        ...
        public virtual ProductModel PrepareProductModel(ProductModel model, Product product, bool excludeProperties = false)
        {
			...
			if (!string.IsNullOrEmpty(product.Custitem_branded_variants))
			{
                var NameItemCollection = "";
                var Items = product.Custitem_branded_variants.Split(",");

				foreach (var item in Items)
				{
					if (!string.IsNullOrEmpty(item.ToString()))
					{
                        var getItemsList = _itemCollection.GetItemCollectionById(Convert.ToInt32(item));

                        if (getItemsList != null)
                        {
                            NameItemCollection += getItemsList.Name + ", ";
                        }
                    }
                    
                }
                model.Custitem_branded_variants = NameItemCollection;
            }
