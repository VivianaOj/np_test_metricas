...
namespace Nop.Web.Areas.Admin.Factories
{
    ...
    public partial class BaseAdminModelFactory : IBaseAdminModelFactory
    {
        ...
        public virtual void PrepareOrderStatuses(IList<SelectListItem> items, bool withSpecialDefaultItem = true, string defaultItemText = null)
        {
            ...
            {
                statusItem.Text = (statusItem.Text == "Pending" ? "Order Processing" : statusItem.Text);
                statusItem.Text = (statusItem.Text == "Processing" ? "Order Confirmed - Pending fulfillment" : statusItem.Text);
                statusItem.Text = (statusItem.Text == "Complete" ? "Completed" : statusItem.Text);
                statusItem.Text = (statusItem.Text == "Completed Billed" ? "Completed and Billed" : statusItem.Text);

                statusItem.Text = (statusItem.Text == "Ready Pickup" ? "Ready for Pickup" : statusItem.Text);
                statusItem.Text = (statusItem.Text == "Ready Delivery" ? "Ready for Delivery" : statusItem.Text);
                statusItem.Text = (statusItem.Text == "Ready Ups" ? "Ready for UPS Pickup" : statusItem.Text);
        ...
        public virtual void PrepareCompanyList(IList<SelectListItem> items, bool withSpecialDefaultItem = true, string defaultItemText = null)
        {
            if (items == null)
                throw new ArgumentNullException(nameof(items));

            //prepare available order statuses
            var availableStatusItems = _companyService.GetAllCompanies();
            foreach (var item in availableStatusItems)
            {
                SelectListItem statusItem = new SelectListItem();

                statusItem.Text = item.CompanyName;
                statusItem.Value = item.Id.ToString();
                items.Add(statusItem);
            }
        ...
        public virtual void PrepareUserRoleList(IList<SelectListItem> items, bool withSpecialDefaultItem = true, string defaultItemText = null)
        {
            if (items == null)
                throw new ArgumentNullException(nameof(items));
        ...
        public virtual void PrepareInvoiceStatuses(IList<SelectListItem> items, bool withSpecialDefaultItem = true, string defaultItemText = null)
        {
            if (items == null)
                throw new ArgumentNullException(nameof(items));


            items.Add(new SelectListItem { Value = "1", Text = "Pending Approval" });

            items.Add(new SelectListItem { Value = "2", Text = "Approved" });
        ...
        public virtual void PreparePaymentStatuses(IList<SelectListItem> items, bool withSpecialDefaultItem = true, string defaultItemText = null)
        {
            ...
            foreach (var statusItem in availableStatusItems)
            {
                statusItem.Text = (statusItem.Text == "Pending" ? "UnPaid" : statusItem.Text);
                
                if (statusItem.Text == "UnPaid" || statusItem.Text == "Paid")
                {
                    items.Add(statusItem);
                }
            }
