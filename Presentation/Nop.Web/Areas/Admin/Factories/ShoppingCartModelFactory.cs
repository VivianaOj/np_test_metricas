    ...
    public partial class ShoppingCartModelFactory : IShoppingCartModelFactory
    {
        ...
        public virtual ShoppingCartListModel PrepareShoppingCartListModel(ShoppingCartSearchModel searchModel)
        {
            ...
            var customers = _customerService.GetCustomersWithShoppingCarts(searchModel.ShoppingCartType,
                ...
                selectedCustomerRoleIds : searchModel.SelectedCustomerRoleIds.ToArray(),
            ...
            var model = new ShoppingCartListModel().PrepareToGrid(searchModel, customers, () =>
            {
                return customers.Select(customer =>
                {
                    ...
                    var itemsList = _shoppingCartService.GetShoppingCart(customer, searchModel.ShoppingCartType,
                        searchModel.StoreId, searchModel.ProductId, searchModel.StartDate, searchModel.EndDate);
                    var dateLastUpdate= itemsList.OrderByDescending(x => x.UpdatedOnUtc).FirstOrDefault();
                    ...
                    shoppingCartModel.TotalItems = itemsList.Sum(item => item.Quantity);
                    
                    shoppingCartModel.CustomerRole = string.Join(", ", customer.CustomerRoles.Select(role => role.Name));
                    shoppingCartModel.LastUpdatedItem = _dateTimeHelper.ConvertToUserTime(dateLastUpdate.UpdatedOnUtc, DateTimeKind.Utc);
        ...
        public virtual ShoppingCartItemListModel PrepareShoppingCartItemListModel(ShoppingCartItemSearchModel searchModel, Customer customer)
        {
            ...
            var model = new ShoppingCartItemListModel().PrepareToGrid(searchModel, items, () =>
            {
                return items.Select(item =>
                {
                    ...
                    itemModel.UnitPrice = _priceFormatter.FormatPrice(_taxService.GetProductPrice(item.Product, unitPrice, out var _, false, decimal.Zero));
                    var subTotal = _priceCalculationService.GetSubTotal(item);
                    itemModel.Total = _priceFormatter.FormatPrice(_taxService.GetProductPrice(item.Product, subTotal, out _, false, decimal.Zero));
