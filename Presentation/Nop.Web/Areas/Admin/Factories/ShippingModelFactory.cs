...
namespace Nop.Web.Areas.Admin.Factories
{
    ...
    public partial class ShippingModelFactory : IShippingModelFactory
    {
        ...
        public ShippingModelFactory(IAddressService addressService,
            ...
            IShippingService shippingService, IStateProvinceService stateProvinceService, IWarehouseLocationNNService warehouseLocationNNService)
        {
        ...
        #region NN Moving Supplies warehouses
        

        public virtual WarehouseLocationNNSearchModel PrepareWarehouseLocationNNSearchModel(WarehouseLocationNNSearchModel searchModel)
        {
            if (searchModel == null)
                throw new ArgumentNullException(nameof(searchModel));
        ...
        public virtual WarehouseLocationNNListModel PrepareNNWarehouseListModel(WarehouseLocationNNSearchModel searchModel)
        {
            if (searchModel == null)
                throw new ArgumentNullException(nameof(searchModel));

            //get warehouses
            var warehousesList = _shippingService.GetAllWarehouses();

            var stateProvinceList = _stateProvinceService.GetStateProvinces();

            var warehouseLocationList = _warehouseLocationNNService.GetWarehouseLocation();
            
            // Create the model and prepare it
            var model = new WarehouseLocationNNListModel
            {
                Data = warehouseLocationList.Select(warehouse => new WarehouseLocationNNModel
                {
                    Name = stateProvinceList.Where(x=>x.Id == warehouse.StateProvideceId).FirstOrDefault()?.Name,
                    // Assuming Location should be included, uncomment and set appropriately
                    Location = warehousesList.Where(x => x.Id == warehouse.WarehouseId).FirstOrDefault()?.Name,
                    Id = warehouse.Id,
                    StateId = warehouse.StateProvideceId
                    
                }).OrderBy(r=>r.Name).ToList(),

                // Fill the state names and warehouse
                StateNames = stateProvinceList.ToList(),
                WarehouseList = warehousesList.ToList()
            };
