    ...
    public partial class CampaignModelFactory : ICampaignModelFactory
    {
        ...
        public virtual CampaignModel PrepareCampaignModel(CampaignModel model, Campaign campaign, bool excludeProperties = false)
        {
            ...
            if (campaign != null)
            {
                ...
                if (campaign.DontSendBeforeDateUtc.HasValue)
                {
                    // Assuming _dateTimeHelper.ConvertToUserTime takes care of time zone conversion
                    // and returns a DateTimeOffset with the correct time zone information.
                    DateTimeOffset userTime = _dateTimeHelper.ConvertToUserTime(campaign.DontSendBeforeDateUtc.Value, DateTimeKind.Utc);

                    // Specify the time zone for Atlanta (Eastern Time Zone)
                    TimeZoneInfo atlantaTimeZone = TimeZoneInfo.FindSystemTimeZoneById("Eastern Standard Time");

                    // Convert the user time to Atlanta time
                    DateTime atlantaTime = TimeZoneInfo.ConvertTime(userTime, atlantaTimeZone).DateTime;

                    // Now 'atlantaTime' contains the converted time in Atlanta time.
                    model.DontSendBeforeDate = atlantaTime;
                }
