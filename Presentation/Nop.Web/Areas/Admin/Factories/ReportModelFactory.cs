...
namespace Nop.Web.Areas.Admin.Factories
{
    ...
    public partial class ReportModelFactory : IReportModelFactory
    {
        ...
        public virtual SearchedWordListModel PrepareSearchedWordListModel(SearchedWordSearchModel searchModel)
        {
            if (searchModel == null)
                throw new ArgumentNullException(nameof(searchModel));
            ...
            //get bestsellers
            var searchedWords = _orderReportService.SearchWordsReport(
                createdFromUtc: startDateValue,
                createdToUtc: endDateValue,
            ...
            //prepare list model
            var model = new SearchedWordListModel().PrepareToGrid(searchModel, searchedWords, () =>
            {
                return searchedWords.Select(searchedWord =>
                {
                    //fill in model values from the entity
                    var searchedWordModel = new SearchedWordModel
                    {
                        Word = searchedWord.Keyword,
                        Count = searchedWord.Count
                    };
        ...
        public virtual SearchedWordSearchModel PrepareSearchedWordSearchModel(SearchedWordSearchModel searchModel)
        {
            if (searchModel == null)
                throw new ArgumentNullException(nameof(searchModel));
    ...
    public class CustomRender : IRender
    {
        public CustomRender(DataUrlLInk url)
        {
            Url = url;
        }

        public DataUrlLInk Url { get; set; }



    }

    public class DataUrlLInk
    {
        private readonly string baseUrl;
        private readonly string queryParam;

        public DataUrlLInk(string baseUrl, string queryParam, bool removeLeadingSlash = false)
        {
            this.baseUrl = removeLeadingSlash ? baseUrl.TrimStart('/') : baseUrl;
