...
namespace Nop.Web.Models.Catalog
{
    ...
    public partial class CatalogPagingFilteringModel : BasePageableModel
    {
        ...
        public partial class PriceRangeFilterModel : BaseNopModel
        {
            ...
            protected virtual IList<PriceRange> GetForPriceRangeList(string priceRangesStr)
            {
                var priceRanges = new List<PriceRange>();
                if (string.IsNullOrWhiteSpace(priceRangesStr))
                    return priceRanges;
                ...
                int leng = rangeArray.Count();

                var fromTo = rangeArray[0].Trim().Split(new[] { '-' });
                decimal? from = null;
                if (!string.IsNullOrEmpty(fromTo[0]) && !string.IsNullOrEmpty(fromTo[0].Trim()))
                    from = decimal.Parse(fromTo[0].Trim(), new CultureInfo("en-US"));
              
                fromTo = rangeArray[leng-1].Trim().Split(new[] { '-' });
                decimal? to = null;
                if (!string.IsNullOrEmpty(fromTo[1]) && !string.IsNullOrEmpty(fromTo[1].Trim()))
                    to = decimal.Parse(fromTo[1].Trim(), new CultureInfo("en-US"));


                for (decimal i =Convert.ToInt32(from); i < Convert.ToInt32(to); i++)
                {
                    priceRanges.Add(new PriceRange { From = i, To = to });
                }
            ...
            public virtual PriceRange GetForSelectedPriceRange(IWebHelper webHelper, string priceRangesStr)
            {
                var range = webHelper.QueryString<string>(QUERYSTRINGPARAM);
                if (string.IsNullOrEmpty(range))
                    return null;
                ...
                if (fromTo.Length == 2)
                {
                    ...
                    var priceRangeList = GetForPriceRangeList(priceRangesStr);
                    foreach (var pr in priceRangeList)
                    {
                        if (pr.From == from && pr.To == to)
                            return pr;
            ...
            public virtual void LoadForPriceRangeFilters(string priceRangeStr, IWebHelper webHelper, IPriceFormatter priceFormatter)
            {
                var priceRangeList = GetForPriceRangeList(priceRangeStr);

                if (priceRangeList.Any())
                {
                    Enabled = true;

                    var selectedPriceRange = GetForSelectedPriceRange(webHelper, priceRangeStr);
