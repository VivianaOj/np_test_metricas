...
namespace Nop.Web.Factories
{
    public partial class CheckoutModelFactory : ICheckoutModelFactory
    {
        ...
        string overrideAttributesXml = "")
        {
            ...
            //existing addresses
            List<Address> addresses = _workContext.CurrentCustomer.Addresses
            ...
            if (_workContext.CurrentCustomer.IsRegistered() &&
                _workContext.CurrentCustomer.Companies.Any())
            {
                //company addresses
                if (_workContext.WorkingCompany != null)
                {
                    var companyAddressMappings = _companyService.GetAllCompanyAddressMappingsById(_workContext.WorkingCompany.Id);

                    var companyAddresses = _workContext.WorkingCompany.Addresses
                        .Where(a => a.Id > 0 && (a.Country == null ||
                            (//published
                            a.Country.Published &&
                            //allow billing
                            a.Country.AllowsBilling &&
                            !a.Active &&
                            a.Email == _workContext.CurrentCustomer.Email &&
                            //enabled for the current store
                            _storeMappingService.Authorize(a.Country)))
                            && companyAddressMappings.Select(cam => cam.AddressId).Contains(a.Id))
                        .ToList();

                    //addresses = companyAddresses.Union(addresses).ToList();

                    var AddressList = new List<Address>();

                    foreach (var item in companyAddresses)
                    {
                        if(!AddressList.Select(a => a.NetsuitId).Contains(item.NetsuitId))
            ...
            else
            {
                ...
                var billing = _workContext.CurrentCustomer.BillingAddress;
                if (billing != null)
                {
                    var addressModel = new AddressModel();
                    _addressModelFactory.PrepareAddressModel(addressModel,
                        ...
                        addressSettings: _addressSettings,
                        loadCountries: () => _countryService.GetAllCountries(_workContext.WorkingLanguage.Id));
                    if (_addressService.IsAddressValid(billing))
                    {
                        model.ExistingAddresses.Add(addressModel);
                    }
                    else
                    ...
                    addresses.Remove(billing);
                }

                var shipping = _workContext.CurrentCustomer.ShippingAddress;
                if (shipping != null)
                {
                    var addressModel = new AddressModel();
                    _addressModelFactory.PrepareAddressModel(addressModel,
                    ...
                    if (_addressService.IsAddressValid(shipping))
                    {
                        model.ExistingAddresses.Add(addressModel);
                    }
                    else
            ...
            //new address
            var addressNoNull = addresses.Where(x => !string.IsNullOrEmpty(x.Address1)).FirstOrDefault();
            if (addressNoNull != null)
            {
                if (model.BillingNewAddress?.CountryId != null)
                    model.BillingNewAddress.CountryId = model.BillingNewAddress?.CountryId;
                else
                    model.BillingNewAddress.CountryId = addressNoNull.CountryId;

            }
            //model.BillingNewAddress.CountryId = addressNoNull != null ? model.BillingNewAddress?.CountryId : addressNoNull.CountryId;
            else
            {
                if (selectedCountryId == null)
                {
                    model.BillingNewAddress.CountryId = 1;
                }
                else
                    model.BillingNewAddress.CountryId = selectedCountryId;
            }
            //Remove adress duplicate
            model.ExistingAddresses = model.ExistingAddresses.GroupBy(x => x.Id).Select(y => y.First()).ToList();

            _addressModelFactory.PrepareAddressModel(model.BillingNewAddress,
                address: addressNoNull,
                excludeProperties: false,
                addressSettings: _addressSettings,
            ...
            bool prePopulateNewAddressWithCustomerFields = false, string overrideAttributesXml = "")
        {
           ...
           // model.ShippingNewAddress.CountryId = selectedCountryId;
            ...
            if (addressNoNull != null)
            {
                if (model.ShippingNewAddress?.CountryId != null)
                    model.ShippingNewAddress.CountryId = model.ShippingNewAddress?.CountryId;
                else
                    model.ShippingNewAddress.CountryId  =addressNoNull.CountryId;
            ...
            else
            {
                if (selectedCountryId == null)
                {
                    model.ShippingNewAddress.CountryId = 1;
                }
                else
                    model.ShippingNewAddress.CountryId = selectedCountryId;
            }
        ...
        public virtual CheckoutShippingMethodModel PrepareShippingMethodModel(IList<ShoppingCartItem> cart, Address shippingAddress)
        {
            ...
            if (getShippingOptionResponse.Success)
            {
                ...
                foreach (var shippingOption in getShippingOptionResponse.ShippingOptions)
                {
                    var soModel = new CheckoutShippingMethodModel.ShippingMethodModel
                    {
                        ...
                        AddressId = shippingOption.AddressId
                    };
                    ...
                    model.IsCommercial = shippingOption.IsCommercial;
                    _genericAttributeService.SaveAttribute(_workContext.CurrentCustomer, NopCustomerDefaults.SelectedShippingOptionAttributeIsCommercial, shippingOption, _storeContext.CurrentStore.Id);

                    model.ShippingMethods.Add(soModel);
                }

                //order 
                model.ShippingMethods = model.ShippingMethods.OrderBy(x => Decimal.Parse(x.Fee.Replace("$",""))).ToList();
            ...
            if (_workContext.CurrentCustomer.NetsuitId != 0)
                model.IsAccoutCustomer = true;
            else 
                model.IsAccoutCustomer = false;

            var UnShippable = cart.Where(r => r.Product.UnShippable == true).ToList();

            if (UnShippable.Count > 0)
                model.UnShippable = true;
        ...
        public virtual OnePageCheckoutModel PrepareOnePageCheckoutModel(IList<ShoppingCartItem> cart)
        {
            ...
            var model = new OnePageCheckoutModel
            {
                ...
                CurrentCustomerIsAccount = _workContext.CurrentCustomer.NetsuitId
            };
            return model;
        }
