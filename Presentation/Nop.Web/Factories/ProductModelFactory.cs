...
namespace Nop.Web.Factories
{
    ...
    public partial class ProductModelFactory : IProductModelFactory
    {
        ...
        public ProductModelFactory(CaptchaSettings captchaSettings,
            ...
            VendorSettings vendorSettings, IAclService aclService)
        {
        ...
        protected virtual void PrepareSimpleProductOverviewPriceModel(Product product, ProductOverviewModel.ProductPriceModel priceModel)
        {
            ...
            if (_permissionService.Authorize(StandardPermissionProvider.DisplayPrices))
            {
                ...
                else
                {
                    ...
                    var oldPriceBase = _taxService.GetProductPrice(product, product.OldPrice, out decimal _, false, decimal.Zero);
                    var finalPriceWithoutDiscountBase = _taxService.GetProductPrice(product, minPossiblePriceWithoutDiscount, out _, false, decimal.Zero);
                    var finalPriceWithDiscountBase = _taxService.GetProductPrice(product, minPossiblePriceWithDiscount, out _, false, decimal.Zero);
        ...
        protected virtual void PrepareGroupedProductOverviewPriceModel(Product product, ProductOverviewModel.ProductPriceModel priceModel)
        {
            ...
            if (_permissionService.Authorize(StandardPermissionProvider.DisplayPrices))
            {
                ...
                {
                    //calculate prices
                    var finalPriceBase = _taxService.GetProductPrice(minPriceProduct, minPossiblePrice.Value, out decimal _, false, decimal.Zero);
        ...
        protected virtual ProductDetailsModel.ProductPriceModel PrepareProductPriceModel(Product product)
        {
            ...
            if (_permissionService.Authorize(StandardPermissionProvider.DisplayPrices))
            {
                ...
                else
                {
                    ...
                    {
                        var oldPriceBase = _taxService.GetProductPrice(product, product.OldPrice, out decimal _, false, decimal.Zero);
                        var finalPriceWithoutDiscountBase = _taxService.GetProductPrice(product, _priceCalculationService.GetFinalPrice(product, _workContext.CurrentCustomer, includeDiscounts: false), out _, false, decimal.Zero);
                        var finalPriceWithDiscountBase = _taxService.GetProductPrice(product, _priceCalculationService.GetFinalPrice(product, _workContext.CurrentCustomer, includeDiscounts: true), out _, false, decimal.Zero);
        ...
        protected virtual ProductDetailsModel.AddToCartModel PrepareProductAddToCartModel(Product product, ShoppingCartItem updatecartitem)
        {
            ...
            if (product.IncrementQuantity > 1)
            {
                model.EnteredQuantity = product.IncrementQuantity;
            }
                model.IncrementQuantity = product.IncrementQuantity;
            model.quantityMessage =product.QuantityMessage;
            model.OutStock = product.OutStock;
                ...
                if (attribute.ShouldHaveValues())
                {
                    ...
                    foreach (var attributeValue in attributeValues)
                    {
                        ...
                        if (_permissionService.Authorize(StandardPermissionProvider.DisplayPrices))
                        {
                            ...
                            var priceAdjustmentBase = _taxService.GetProductPrice(product, attributeValuePriceAdjustment, out decimal _, false, decimal.Zero);
        ...
        protected virtual IList<ProductDetailsModel.TierPriceModel> PrepareProductTierPriceModels(Product product)
        {
            ...
            var model = product.TierPrices.OrderBy(x => x.Quantity)
                   ...
                   .Select(tierPrice =>
                   {
                       ...
                       _workContext.CurrentCustomer, decimal.Zero, _catalogSettings.DisplayTierPricesWithDiscounts, tierPrice.Quantity), out decimal _, false, decimal.Zero);
            ...
            bool forceRedirectionAfterAddingToCart = false)
        {
            ...
            {
                var itemCollection = _aclService.AuthorizeItemCollection(product);
                if (!itemCollection)
                    continue;
                ...
                foreach (var item in product.ProductCategories)
                {
                    ...
                    var categorybread = _categoryService.GetCategoryById(item.Category.Id);
                    //category breadcrumb
                    if (_catalogSettings.CategoryBreadcrumbEnabled)
                        {
                            ...
                            var breadcrumbCacheKey = string.Format(NopModelCacheDefaults.CategoryBreadcrumbKey,
                                ...
                                _categoryService.GetCategoryBreadCrumb(categorybread).Select(catBr => new CategoryModel
                                {
                                    Id = catBr.Id,
                                    Name = _localizationService.GetLocalized(catBr, x => x.Name),
                                    SeName = _urlRecordService.GetSeName(catBr)
                ...
                model.Custitem_branded_variants = product.Custitem_branded_variants;
                //specs
            ...
            ShoppingCartItem updatecartitem = null, bool isAssociatedProduct = false)
        {
            ...
            var model = new ProductDetailsModel
            {
                ...
                Height = Math.Truncate(product.Height),
                Width = Math.Truncate(product.Width),
                Length = Math.Truncate(product.Length),
                LoadCapacity = Math.Truncate(product.LoadCapacity),
                ShipLength = product.ShipLength,
                ShipWidth = product.ShipWidth,
                ShipHeight = product.ShipHeight,
                SpecificationClass = product.SpecificationClass,
