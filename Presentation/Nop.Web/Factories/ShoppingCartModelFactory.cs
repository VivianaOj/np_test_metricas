...
namespace Nop.Web.Factories
{
    ...
    public partial class ShoppingCartModelFactory : IShoppingCartModelFactory
    {
        ...
        public ShoppingCartModelFactory(AddressSettings addressSettings,
            ...
            IOrderService orderService, IAclService aclService)
        {
        ...
        protected virtual ShoppingCartModel.ShoppingCartItemModel PrepareShoppingCartItemModel(IList<ShoppingCartItem> cart, ShoppingCartItem sci, IList<Vendor> vendors)
        {
            ...
            var IncrementQuantity = sci.Product.IncrementQuantity;
            var quantityMessage = sci.Product.QuantityMessage;
            ...
            var cartItemModel = new ShoppingCartModel.ShoppingCartItemModel
            {
                ...
                UnShippable=sci.Product.UnShippable,
                IncrementQuantity = IncrementQuantity,
                quantityMessage = quantityMessage,
                OutStock = sci.Product.OutStock
        };
            ...
            {
                var shoppingCartUnitPriceWithDiscountBase = _taxService.GetProductPrice(sci.Product, _priceCalculationService.GetUnitPrice(sci), out var _, false, decimal.Zero);
                ...
                int id = Convert.ToInt32(sci.ProductId);
                //Validate ItemPricing
                if (_workContext.WorkingCompany?.ItemsPricing != null && _workContext.WorkingCompany?.ItemsPricing.FirstOrDefault(s => s.ProductId == id) != null)
                {
                    var qty = sci.Product.PriceByQtyProduct.Where(s => sci.Quantity >= s.Quantity).OrderByDescending(s => s.Quantity);
                    var getPricebyCompany = _workContext.WorkingCompany.ItemsPricing.Where(s => s.ProductId == sci.Product.Id);
                    var pricelevelId = qty.Where(r => getPricebyCompany.Any(p => p.PriceLevel == r.PriceLevelId));

                    if (pricelevelId.Count() > 0)
                        price = pricelevelId.Where(s => sci.Quantity >= s.Quantity).FirstOrDefault().Price;
                    else
                        price = _workContext.WorkingCompany.ItemsPricing.FirstOrDefault(s => s.ProductId == id).Price;
                ...
                else
                {
                    ...
                    if (_workContext.CurrentCustomer.CustomerRoles.FirstOrDefault(s => s.Id == 4) != null || (_workContext.CurrentCustomer.NetsuitId == 0 && _workContext.CurrentCustomer.IsRegistered()))
                    {
                        //2= Price Level Retail & Web Store
                        if (sci.Product.PriceByQtyProduct.Where(s => s.PriceLevelId == 2 && sci.Quantity >= s.Quantity).OrderByDescending(s => s.Quantity).FirstOrDefault() != null)
                            price = sci.Product.PriceByQtyProduct.Where(s => s.PriceLevelId == 2 && sci.Quantity >= s.Quantity).OrderByDescending(s => s.Quantity).FirstOrDefault().Price;
                        else
                            price = sci.Product.Price;
                    }
                    else
                    {
                        if (_workContext.WorkingCompany?.PriceLevelId != null && sci.Product.PriceByQtyProduct.Where(s => s.PriceLevelId == _workContext.WorkingCompany?.PriceLevelId && sci.Quantity >= s.Quantity).OrderByDescending(s => s.Quantity).FirstOrDefault() != null)
                            price = sci.Product.PriceByQtyProduct.Where(s => s.PriceLevelId == _workContext.WorkingCompany?.PriceLevelId && sci.Quantity >= s.Quantity).OrderByDescending(s => s.Quantity).FirstOrDefault().Price;
                        else
                        {
                            if (_workContext.WorkingCompany?.PriceLevelId != null && sci.Product.PriceByQtyProduct.Where(s => sci.Quantity >= s.Quantity).OrderByDescending(s => s.Quantity).FirstOrDefault() != null)
                                price = sci.Product.PriceByQtyProduct.Where(s => sci.Quantity >= s.Quantity).OrderByDescending(s => s.Quantity).FirstOrDefault().Price;
                        }
                    }

                }
                if(sci.Product.OldPrice!=0)
                    cartItemModel.OldPrice = _priceFormatter.FormatPrice(sci.Product.OldPrice);
                cartItemModel.UnitPrice = _priceFormatter.FormatPrice(price);
                cartItemModel.SubTotal = _priceFormatter.FormatPrice(price * sci.Quantity);
            }
            ...
            {
                //sub total
                var shoppingCartItemSubTotalWithDiscountBase = _taxService.GetProductPrice(sci.Product, _priceCalculationService.GetSubTotal(sci, true, out var shoppingCartItemDiscountBase, out var _, out var maximumDiscountQty), out var taxRate,false, decimal.Zero);
                ...
                {
                    shoppingCartItemDiscountBase = _taxService.GetProductPrice(sci.Product, shoppingCartItemDiscountBase, out taxRate, false, decimal.Zero);
                    if (shoppingCartItemDiscountBase > decimal.Zero)
                    {
            ...
            //Catergory 
            var categories = _categoryService.GetProductCategoriesByProductId(sci.ProductId);
            if (categories != null && categories.Any())
            {
                var category = categories.FirstOrDefault();
                cartItemModel.CategoryName = category.Category.Name;
            }
        ...
        protected virtual WishlistModel.ShoppingCartItemModel PrepareWishlistItemModel(IList<ShoppingCartItem> cart, ShoppingCartItem sci)
        {
            ...
            var cartItemModel = new WishlistModel.ShoppingCartItemModel
            {
                ...
                ProductSeName = _urlRecordService.GetSeName(sci.Product),
                Quantity = sci.Quantity,
                IncrementQuantity = sci.Product.IncrementQuantity,
                AttributeInfo = _productAttributeFormatter.FormatAttributes(sci.Product, sci.AttributesXml),
            };
        ...
        public virtual ShoppingCartModel PrepareShoppingCartModel(ShoppingCartModel model,
            ...
            bool prepareAndDisplayRelatedProducts = false)
        {
            if (cart == null)
                throw new ArgumentNullException(nameof(cart));
            ...
            foreach (var item in cart)
            {
                    ...
                    var itemCollection = _aclService.AuthorizeItemCollection(item.Product);
                    if (!itemCollection)
                    {
                        item.Product.Published = false;
                        item.Product.ItemCollectionStatus = true;
                        item.Product.ProductCost = item.Product.Price;
                        item.Product.Price = 0;
                    }
                    else
                    {
                        if (item.Product.ItemCollectionStatus && item.Product.Published==false)
                        {
                            item.Product.Published = true;
                            item.Product.ItemCollectionStatus = false;

                            if (item.Product.Price == 0)
                            {
                                item.Product.Price = item.Product.ProductCost;
                            }
                        }

                        //if (item.Product.ItemCollectionStatus==false && item.Product.Published == false)
                        //{
                        //    item.Product.Published = true;
                        //    item.Product.ItemCollectionStatus = false;
            ...
            // MiniShopping Cart
            model.DisplayShoppingCartButton = true;
            model.CurrentCustomerIsGuest = _workContext.CurrentCustomer.IsGuest();
            model.AnonymousCheckoutAllowed = _orderSettings.AnonymousCheckoutAllowed;
            ...
            if (cart.Any())
            {
                ...
                model.SubTotal = subtotal.ToString("G15");
               
                //var total = Math.Truncate(subtotalBase * 100) / 100;
                //model.SubTotal = total.ToString();
            ...
            if (prepareAndDisplayRelatedProducts)
            {
                model.ProductIds = PrepareProductsAlsoPurchased(cart);
            }
        ...
        /// <summary>
        /// PrepareProductsAlsoPurchased
        /// </summary>
        /// <param name="cart"></param>
        /// <returns></returns>
        public virtual List<ProductOverviewModel> PrepareProductsAlsoPurchased(IList<ShoppingCartItem> cart)
        {
            var productList = new List<ProductOverviewModel>();
            ...
            var orders = _orderService.SearchOrders(customer.Id);
            foreach (var item in orders.Select(x => x.OrderItems).ToList())
            {
                foreach (var product in item.Select(x => x.Product).Distinct())
                {
                    var catalogProduct = new ProductOverviewModel();
                    catalogProduct.Id = product.Id;
                    catalogProduct.Name = product.Name;
                    catalogProduct.FullDescription = product.FullDescription;
                    catalogProduct.SeName = product.Name;
                    catalogProduct.ProductType = product.ProductType;

                    //var taxStr = _priceFormatter.FormatPrice(product.Price, true, customer.curr, false);
                    catalogProduct.ProductPrice = new ProductOverviewModel.ProductPriceModel()
                    {
            ...
            // Get relatedProducts by productId
            if (!productList.Any())
            {
                foreach (var item in cart)
                {
                    var existingRelatedProducts = _productService.GetRelatedProductsByProductId1(item.ProductId);

                    foreach (var product in existingRelatedProducts.Select(x => x.ProductId1).Distinct())
                    {
                        var productRelated = _productService.GetProductById(product);

                        var catalogProduct = new ProductOverviewModel();
                        catalogProduct.Id = productRelated.Id;
                        catalogProduct.Name = productRelated.Name;
                        catalogProduct.FullDescription = productRelated.FullDescription;
                        catalogProduct.SeName = productRelated.Name;
                        catalogProduct.ProductType = productRelated.ProductType;

                        //var taxStr = _priceFormatter.FormatPrice(product.Price, true, customer.curr, false);
                        catalogProduct.ProductPrice = new ProductOverviewModel.ProductPriceModel()
                        {
                            Price = productRelated.Price.ToString()
                        };
        ...
        public virtual MiniShoppingCartModel PrepareMiniShoppingCartModel()
        {
            ...
            if (_workContext.CurrentCustomer.HasShoppingCartItems)
            {
                ...
                if (cart.Any())
                {
                    ...
                    var subtotal = _currencyService.ConvertFromPrimaryStoreCurrency(subtotalBase, _workContext.WorkingCurrency);
                    model.SubTotal = _priceFormatter.FormatPrice(subtotal, false, _workContext.WorkingCurrency, _workContext.WorkingLanguage, subTotalIncludingTax);
                    model.SubTotalValue = subtotalBase;
        ...
        public virtual OrderTotalsModel PrepareOrderTotalsModel(IList<ShoppingCartItem> cart, bool isEditable)
        {
            ...
            if (cart.Any())
            {
                ...
                if (model.RequiresShipping)
                {
                    ...
                    {
                         shoppingCartShipping = _currencyService.ConvertFromPrimaryStoreCurrency(shoppingCartShippingBase.Value, _workContext.WorkingCurrency);
                ...
                {
                    var shoppingCartTaxBase = _orderTotalCalculationService.GetTaxTotal(cart, shippingRateComputationMethods, out var taxRates,false, shoppingCartShipping);
                ...
                if (shoppingCartTotalBase.HasValue)
                {
                    ...
                    //model.OrderTotal = shoppingCartTotal.ToString("G15");

                    //var total2 = Math.Truncate(shoppingCartTotalBase.Value * 100) / 100;
