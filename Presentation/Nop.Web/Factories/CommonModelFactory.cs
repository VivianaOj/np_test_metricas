...
namespace Nop.Web.Factories
{
    ...
    public partial class CommonModelFactory : ICommonModelFactory
    {
        ...
        public CommonModelFactory(BlogSettings blogSettings,
            ...
            VendorSettings vendorSettings, IItemCollectionServices itemCollection)
        {
        ...
        /// <summary>
        /// Prepare the company selector model
        /// </summary>
        /// <returns>Currency selector model</returns>
        public virtual CompanySelectorModel PrepareCompanySelectorModel()
        {
            ...
            {
                var companies = _workContext.CurrentCustomer.Companies;
                var Comp = _workContext.CurrentCustomer.CompanyCustomerMappings.Where(r=>r.Active).ToList();


                if (Comp.Count() == 1)
                {
                    var UniqueCompany = _workContext.CurrentCustomer.CompanyCustomerMappings.FirstOrDefault(x => x.DefaultCompany == false && x.Active);

                    if(UniqueCompany!=null)
                    {
                        var firstCompany = UniqueCompany.Company;

                        if (firstCompany != null)
                        {
                            model.CurrentCompanyId = firstCompany.Id;
                            UniqueCompany.DefaultCompany = true;
                            //set the first comapny as default
                            _workContext.WorkingCompany = firstCompany;
                        }
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    {
                        var company = _workContext.CurrentCustomer.CompanyCustomerMappings.Where(r => r.DefaultCompany && r.Active).FirstOrDefault();

                        if (company == null)
                            model.CurrentCompanyId = _workContext.WorkingCompany.Id;
                        else
                            model.CurrentCompanyId = company.CompanyId;
                    }

                }
                var availableCompanies = Comp.Select(x =>
                {
        ...
        public virtual HeaderLinksModel PrepareHeaderLinksModel()
        {
            ...
            var model = new HeaderLinksModel
            {
                ...
                Name = _genericAttributeService.GetAttribute<string>(customer, NopCustomerDefaults.FirstNameAttribute)
        };
        ...
        public virtual AdminHeaderLinksModel PrepareAdminHeaderLinksModel()
        {
			...
			{
                var company = customer.CompanyCustomerMappings.Where(x => x.DefaultCompany).FirstOrDefault();
				if (company != null)
				{
                    var listItems = _itemCollection.GetItemCollectionCompanyByNetsuiteId(company.CompanyId);
            ...
            var model = new AdminHeaderLinksModel
            {
                ...
                ItemsCollection = isAccountCustomer
            };
        ...
        public virtual ContactUsModel PrepareContactUsModel(ContactUsModel model, bool excludeProperties)
        {
            ...
            if (!excludeProperties)
            {
                ...
                model.FullName = _genericAttributeService.GetAttribute<string>(_workContext.CurrentCustomer, NopCustomerDefaults.FirstNameAttribute);
                model.LastName = _genericAttributeService.GetAttribute<string>(_workContext.CurrentCustomer, NopCustomerDefaults.LastNameAttribute);
            }
