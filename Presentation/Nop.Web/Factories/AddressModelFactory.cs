...
namespace Nop.Web.Factories
{
    ...
    public partial class AddressModelFactory : IAddressModelFactory
    {
        ...
        public AddressModelFactory(AddressSettings addressSettings,
            ...
            IStateProvinceService stateProvinceService, IWorkContext workContext, ICompanyService companyServices)
        {
            ...
            string overrideAttributesXml = "")
        {
            ...
            if (!excludeProperties && address != null)
            {
                ...
                if (_workContext.WorkingCompany != null && _workContext.WorkingCompany?.Id != 0)
                {
                    ...
                    {
                        var addressNN = companyAddress.Where(r => r.AddressId == address.Id).FirstOrDefault();

                        if (addressNN != null)
                            model.DeliveryRouteName = addressNN.DeliveryRouteName;
                    }
                }
            ...
            if (_workContext.CurrentCustomer.BillingAddress?.Id == address?.Id)
                model.IsBilling = true;

            if (_workContext.CurrentCustomer.ShippingAddress?.Id == address?.Id)
                model.IsShipping = true;

            if (address?.Residential==true)
                model.Residential = true;
