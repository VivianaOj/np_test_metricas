...
namespace Nop.Web.Factories
{
    public partial class CatalogModelFactory : ICatalogModelFactory
    {
        ...
        public virtual CategoryModel PrepareCategoryModel(Category category, CatalogPagingFilteringModel command)
        {
            ...
            //price ranges
            model.PagingFilteringContext.PriceRangeFilter.LoadForPriceRangeFilters(category.PriceRanges, _webHelper, _priceFormatter);
            ...
            //{
            //    if( item.ProductPrice.PriceValue > maxPriceConverted)
            //    {
        ...
        public virtual CategoryNavigationModel PrepareCategoryNavigationAccordeonModel()
        {
            ...
            var categories = allCategories.Where(s => s.ParentCategoryId == 0).ToList();
            foreach (var category in categories)
            {
                ...
                var subCategories = PrepareCategorySimpleModels(category.Id, true);
            ...
            string rangePrice = _settingService.GetSetting("catalogsettings.rangespricefilterproduct").Value;
            model.PagingFilteringContext.PriceRangeFilter.LoadForPriceRangeFilters(rangePrice, _webHelper, _priceFormatter);
        ...
        public virtual CategoryNavigationModel PrepareCategoryNavigationModel(int currentCategoryId, int currentProductId)
        {
            ...
            if (cachedCategoriesModel.Count == 0)
                cachedCategoriesModel = PrepareCategorySimpleModels();

            //   var cachedCategoriesModel = new List<CategorySimpleModel>() { GetCategorySimpleModelById(currentCategoryId) };
                    ...
                    if (true)
                    {
                        ...
                        catModel.SubCategories = _cacheManager.Get(subCategoriesCacheKey, () =>
                            ...
                            .Select(x =>
                            {
                                ...
                                subCatModel.SubCategories = _categoryService.GetAllCategoriesByParentCategoryId(x.Id).Select(y =>
                                    {
                                        var subSubCatModel = new CategoryModel.SubCategoryModel
                                        {
                                            Id = y.Id,
                                            Name = _localizationService.GetLocalized(y, c => c.Name),
                                            SeName = _urlRecordService.GetSeName(y),
                                            Description = _localizationService.GetLocalized(x, c => c.Description)
                                        };
                                        return subSubCatModel;
                                    }).ToList();

                                //prepare picture model
                                var categoryPictureCacheKeys = string.Format(NopModelCacheDefaults.CategoryPictureModelKey, x.Id, pictureSize, true, _workContext.WorkingLanguage.Id, _webHelper.IsCurrentConnectionSecured(), _storeContext.CurrentStore.Id);
                                subCatModel.PictureModel = _cacheManager.Get(categoryPictureCacheKeys, () =>
                                {
                                    var picture = _pictureService.GetPictureById(x.PictureId);
                                    var pictureModel = new PictureModel
                                    {
        ...
        public virtual CategorySimpleModel GetCategorySimpleModelById(int idCategory)
        {
            var allCategories = _categoryService.GetAllCategories(storeId: _storeContext.CurrentStore.Id);
            var currentCategory = allCategories.FirstOrDefault(s => s.Id == idCategory);
            var categoryModel = new CategorySimpleModel
            {
                Id = currentCategory.Id,
                Name = _localizationService.GetLocalized(currentCategory, x => x.Name),
                SeName = _urlRecordService.GetSeName(currentCategory),
                IncludeInTopMenu = currentCategory.IncludeInTopMenu
            };

            var dataSubcategories = PrepareCategorySimpleModels(categoryModel.Id, true);

            if (dataSubcategories != null && dataSubcategories.Count > 0)
                categoryModel.SubCategories.AddRange(dataSubcategories.ToArray());
        ...
        public virtual List<CategorySimpleModel> PrepareCategorySimpleModels(int rootCategoryId, bool loadSubCategories = true)
        {
            ...
            foreach (var category in categories)
            {
                ...
                if (loadSubCategories && rootCategoryId != 0)
                {
        ...
        public virtual SearchModel PrepareSearchModel(SearchModel model, CatalogPagingFilteringModel command)
        {
            ...
            if (isSearchTermSpecified)
            {
                ...
                else
                {
                    ...
                    var product_1 = products.OrderByDescending(p =>
                    {
                        int relevance = 0;

                        // Check relevance based on keyword matching in name
                        if (p.Name.ToLower().Contains(searchTerms.ToLower()))
                            relevance += 4;

                        // Check relevance based on keyword matching in tags
                        else if (p.ProductProductTagMappings.Any(tag => tag.ProductTag.Name.ToLower().Contains(searchTerms.ToLower())))
                            relevance += 3;

                        // Check relevance based on keyword matching in description
                        else if (p.ShortDescription.ToLower().Contains(searchTerms.ToLower()))
                            relevance += 2;

                        // Add relevance based on keyword matching in full description
                        else if (p.FullDescription.ToLower().Contains(searchTerms.ToLower()))
                            relevance += 1;
                    ...
                    model.Products = _productModelFactory.PrepareProductOverviewModels(product_1).ToList();
					// var totalProducts = _productModelFactory.PrepareProductOverviewModels(products).ToList();

					var allItems = _productService.SearchProducts(
					   categoryIds: categoryIds,
					   manufacturerId: manufacturerId,
					   storeId: _storeContext.CurrentStore.Id,
					   visibleIndividuallyOnly: true,
					...
					var totalProducts = _productModelFactory.PrepareProductOverviewModels(allItems).ToList();

					totalItems = totalProducts.Count();
					//totalItems = products.Count();

                    model.PagingFilteringContext.TotalPages = products.TotalPages;
                    model.PagingFilteringContext.TotalItems = products.TotalCount;

                    model.NoResults = !model.Products.Any();

                    //Excep List
                    IPagedList<Product> excepProducts = new PagedList<Product>(products, products.PageIndex, products.PageSize, products.TotalCount);
                    ...
                    foreach (var item in model.Products)
                    {
                        ...
                        if (products != null)
                        {
                            excepProducts.Add(product);
                        }
                    }

                    //products = new PagedList<Product>(products, products.PageIndex, products.PageSize);

                    products = excepProducts;
                    ...
                    if (!string.IsNullOrEmpty(searchTerms))
                    {
                        ...
                        if (searchTerm != null)
                        {
                            searchTerm.CreatedOnUtc = DateTime.Now;
                            searchTerm.Count++;
            ...
            model.PagingFilteringContext.TotalItems = totalItems;
            return model;
        }
