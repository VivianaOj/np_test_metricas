...
namespace Nop.Web.Framework
{
    ...
    public partial class WebWorkContext : IWorkContext
    {
        ...
        /// <summary>
        /// Gets or sets the company selected by the customer
        /// </summary>
        public virtual Company WorkingCompany
        {
            get
            {
                if (_cachedCompany == null)
                {
                    var companyId = _genericAttributeService
                        .GetAttribute<string>(CurrentCustomer, NopCustomerDefaults.Company);

                    if (!string.IsNullOrEmpty(companyId))
                    {
                        int companyIntId = 0;
                        if (int.TryParse(companyId, out companyIntId))
                        {
                            var company = CurrentCustomer.Companies?.FirstOrDefault(c => c.Id == companyIntId);
            ...
            set
            {
                _genericAttributeService.SaveAttribute(CurrentCustomer, NopCustomerDefaults.Company, value.Id);

                _cachedCompany = value;
            }
        }
