...
namespace Nop.Services.Security
{
    ...
    public partial class AclService : IAclService
    {
        ...
        public AclService(CatalogSettings catalogSettings,
            ...
            IWorkContext workContext, IItemCollectionServices itemCollection)
        {
        ...
        public virtual bool Authorize<T>(T entity, Customer customer) where T : BaseEntity, IAclSupported
        {
            ...
            var ValidItem = ValidateItemCollection(entity, _workContext.CurrentCustomer);
        ...
        /// <summary>
        /// Authorize ACL permission Validate Item Collection
        /// </summary>
        /// <typeparam name="T">Type</typeparam>
        /// <param name="entity">Entity</param>
        /// <param name="customer">Customer</param>
        /// <returns>true - authorized; otherwise, false</returns>
        public virtual bool AuthorizeItemCollection<T>(T entity) where T : BaseEntity, IAclSupported
        {
            return ValidateItemCollection(entity, _workContext.CurrentCustomer);
        ...
        /// <param name="customer">Customer</param>
        /// <returns>true - authorized; otherwise, false</returns>
        private bool ValidateItemCollection<T>(T entity, Customer customer) where T : BaseEntity, IAclSupported
        {
            var itemCollectionProduct = _itemCollection.GetItemCollectionProductById(entity.Id);
            var InvalidItem = false;
            if (itemCollectionProduct.Count > 0)
            {
                if (customer.NetsuitId != 0)
                {
                    foreach (var item in itemCollectionProduct)
                    {
                        if(_workContext.WorkingCompany!=null)
                        {
                            var companyId = Convert.ToInt32(_workContext.WorkingCompany?.Id) ;
                            var itemCollection = _itemCollection.GetItemCollectionCompanyByNetsuiteId(companyId).Where(x => x.CollectionId == item.CollectionId).FirstOrDefault();
