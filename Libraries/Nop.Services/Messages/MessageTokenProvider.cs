...
namespace Nop.Services.Messages
{
    ...
    public partial class MessageTokenProvider : IMessageTokenProvider
    {
        ...
        public MessageTokenProvider(CatalogSettings catalogSettings,
            ...
            IShippingPluginManager shippingPluginManager, IInvoiceService invoiceService, IPriceCalculationService priceCalculationService
            )
        {
        ...
        protected Dictionary<string, IEnumerable<string>> AllowedTokens
        {
            get
            {
                ...
                _allowedTokens.Add(TokenGroupNames.OrderTokens, new[]
                {
                    ...
                    "%Order.ResidentialAddress%",
                    "%Order.TractorAccessible%",
                    "%Order.trailerEnterExit%",
                    "%Order.StandardDock%",
                    "%Order.AppointmentDelivery%",
                    "%Order.StandardReceiving%",
        ...
        protected virtual string NewProductListToHtmlTable(Order order, int languageId, int vendorId, IList<ShoppingCartItem> cart = null, bool totals = true, bool isfreightQuote=false)
        {
            var productService = EngineContext.Current.Resolve<IProductService>();
            var language = _languageService.GetLanguageById(languageId);
            ...
            for (var i = 0; i <= table.Count - 1; i++)
            {
                ...
                // Quantity
                sb.AppendLine($"<tr>");
                sb.AppendLine($"<td style='padding: .75rem; vertical-align: top;'>{orderItem.Quantity}</td>");

                //attributes 
                if (!string.IsNullOrEmpty(orderItem.AttributeDescription))
                {
                    var productNames = _localizationService.GetLocalized(product, x => x.Name, languageId);
                    sb.AppendLine($"<td style='padding: .75rem; vertical-align: top;width: 300px;'>" + WebUtility.HtmlEncode(productNames));
                    sb.AppendLine("<br />");
                    sb.AppendLine(orderItem.AttributeDescription);
                    sb.AppendLine("</td>");
                }
                else
                {
                    //product name
                    var productName = _localizationService.GetLocalized(product, x => x.Name, languageId);
                    sb.AppendLine($"<td style='padding: .75rem; vertical-align: top;width: 300px;'>" + WebUtility.HtmlEncode(productName) + "</td>");
                }
                ...
                if (orderItem.Product.DiscountProductMappings.Any())
                {
                    var discountMapping = orderItem.Product.DiscountProductMappings.FirstOrDefault();
                    if (discountMapping != null)
                    {
                        if (discountMapping.Discount.DiscountAmount != 0 && discountMapping.Discount.EndDateUtc <= DateTime.Now)
                            discount = discountMapping.Discount.DiscountAmount;

                        if (discountMapping.Discount.DiscountPercentage != 0 && discountMapping.Discount.EndDateUtc > DateTime.Now)
                            discountPercentage = discountMapping.Discount.DiscountPercentage / 100;
                    }
                ...
                //initial price
                decimal price = orderItem.Product.Price;
                ...
                else
                {
                    ...
                    if (customerInfo.CustomerRoles.FirstOrDefault(s => s.Id == 4) != null || (customerInfo.NetsuitId == 0 && customerInfo.IsRegistered()))
                    {
                        //2= Price Level Retail & Web Store
                        if (product.PriceByQtyProduct.Where(s => s.PriceLevelId == 2 && orderItem.Quantity >= s.Quantity).OrderByDescending(s => s.Quantity).FirstOrDefault() != null)
                            price = product.PriceByQtyProduct.Where(s => s.PriceLevelId == 2 && orderItem.Quantity >= s.Quantity).OrderByDescending(s => s.Quantity).FirstOrDefault().Price;
                        else
                            price = product.Price;
                    }
                    else
                    {
                        if (companyInfo?.PriceLevelId != null && product.PriceByQtyProduct.Where(s => s.PriceLevelId == companyInfo?.PriceLevelId && orderItem.Quantity >= s.Quantity).OrderByDescending(s => s.Quantity).FirstOrDefault() != null)
                            price = product.PriceByQtyProduct.Where(s => s.PriceLevelId == companyInfo?.PriceLevelId && orderItem.Quantity >= s.Quantity).OrderByDescending(s => s.Quantity).FirstOrDefault().Price;
                        else
                            price = product.Price;
                    }

                }
                var discountTotalUnit = "";
                var discountTotalUnitTotal = "";
                decimal unitPriceInclTaxInCustomerCurrency = 0;
                decimal unitPriceExclTaxInCustomerCurrency = 0;
                var DiscountTotalSum = "";
                var UnitPriceWithoutDisc = _priceFormatter.FormatPrice(price, true, order.CustomerCurrencyCode, language, false);

                if (order.CustomerTaxDisplayType == TaxDisplayType.IncludingTax)
                {
                    //unitPriceInclTaxInCustomerCurrency = _currencyService.ConvertCurrency(price, order.CurrencyRate);
                    //unitPriceStr = _priceFormatter.FormatPrice(unitPriceInclTaxInCustomerCurrency, true, order.CustomerCurrencyCode, language, false);
                    //unitTotalStr = _priceFormatter.FormatPrice(unitPriceInclTaxInCustomerCurrency * orderItem.Quantity, true, order.CustomerCurrencyCode, language, true);

                    //////excluding tax
                    unitPriceExclTaxInCustomerCurrency = _currencyService.ConvertCurrency(price, order.CurrencyRate);
                    unitPriceStr = _priceFormatter.FormatPrice(unitPriceExclTaxInCustomerCurrency, true, order.CustomerCurrencyCode, language, false);
                    unitTotalStr = _priceFormatter.FormatPrice(unitPriceExclTaxInCustomerCurrency * orderItem.Quantity, true, order.CustomerCurrencyCode, language, true);
                    UnitPriceWithoutDisc = unitPriceStr;

                    if (discount != 0)
                    {
                        unitPriceExclTaxInCustomerCurrency = _currencyService.ConvertCurrency(price, order.CurrencyRate);
                        var discountTotalUnitVal = (price - discount);

                        var DiscountUnit = unitPriceExclTaxInCustomerCurrency - discount;
                        discountTotalUnit = _priceFormatter.FormatPrice((DiscountUnit), true, order.CustomerCurrencyCode, language, true);
                        unitPriceStr = discountTotalUnit;

                        var DiscountTotal = (unitPriceExclTaxInCustomerCurrency - discount) * orderItem.Quantity;
                        discountTotalUnitTotal = _priceFormatter.FormatPrice((unitPriceExclTaxInCustomerCurrency * orderItem.Quantity) - (DiscountTotal), true, order.CustomerCurrencyCode, language, true);
                        unitTotalStr = _priceFormatter.FormatPrice(DiscountTotal, true, order.CustomerCurrencyCode, language, true);

                        DiscountTotalSum = _priceFormatter.FormatPrice((unitPriceExclTaxInCustomerCurrency * orderItem.Quantity) - (DiscountTotal), true, order.CustomerCurrencyCode, language, true); ;
                        UnitPriceWithoutDisc = UnitPriceWithoutDisc;

                    }
                    else if (discountPercentage != 0)
                    {
                        unitPriceExclTaxInCustomerCurrency = _currencyService.ConvertCurrency(price, order.CurrencyRate);
                        var discountTotalUnitVal = (price * discountPercentage);

                        var DiscountUnit = unitPriceExclTaxInCustomerCurrency - discountTotalUnitVal;
                        discountTotalUnit = _priceFormatter.FormatPrice((DiscountUnit), true, order.CustomerCurrencyCode, language, true);
                        unitPriceStr = discountTotalUnit;

                        var DiscountTotal = (discountPercentage * unitPriceExclTaxInCustomerCurrency) * orderItem.Quantity;
                        discountTotalUnitTotal = _priceFormatter.FormatPrice((unitPriceExclTaxInCustomerCurrency * orderItem.Quantity) - (DiscountTotal), true, order.CustomerCurrencyCode, language, true);
                        DiscountTotalSum  = _priceFormatter.FormatPrice(DiscountTotal, true, order.CustomerCurrencyCode, language, true);

                        unitTotalStr = _priceFormatter.FormatPrice((unitPriceExclTaxInCustomerCurrency * orderItem.Quantity) - (DiscountTotal), true, order.CustomerCurrencyCode, language, true); ;
                        UnitPriceWithoutDisc = UnitPriceWithoutDisc;


                    }
                    //unitPriceStr = _priceFormatter.FormatPrice(unitPriceInclTaxInCustomerCurrency, true, order.CustomerCurrencyCode, language, true);
                    // discountTotal = _priceFormatter.FormatPrice(discount * orderItem.Quantity, true, order.CustomerCurrencyCode, language, true);

                    //var unitTotalInclTaxInCustomerCurrency = _currencyService.ConvertCurrency(orderItem.Quantity * (unitPriceInclTaxInCustomerCurrency - discount), order.CurrencyRate);
                    //unitTotalStr = _priceFormatter.FormatPrice(unitPriceInclTaxInCustomerCurrency, true, order.CustomerCurrencyCode, language, true);
                }
                else
                {
                    ////excluding tax
                    ...
                    if (discount != 0)
                    {
                        ...
                        var DiscountTotal = (price - discount ) * orderItem.Quantity;
                        discountTotalUnitTotal = _priceFormatter.FormatPrice((unitPriceExclTaxInCustomerCurrency * orderItem.Quantity) - (DiscountTotal), true, order.CustomerCurrencyCode, language, true);
                        unitTotalStr = _priceFormatter.FormatPrice(DiscountTotal, true, order.CustomerCurrencyCode, language, true);
                    ...
                    //    discountTotal = _priceFormatter.FormatPrice(discount * orderItem.Quantity, true, order.CustomerCurrencyCode, language, true);
                    //else if (discountPercentage != 0)
                    //    discountTotal = _priceFormatter.FormatPrice((discountPercentage * unitPriceExclTaxInCustomerCurrency) * orderItem.Quantity, true, order.CustomerCurrencyCode, language, true);

                    //var unitTotalExclTaxInCustomerCurrency = _currencyService.ConvertCurrency(orderItem.Quantity * (unitPriceExclTaxInCustomerCurrency - discount), order.CurrencyRate);

                }

                sb.AppendLine($"<td style='padding: .75rem; vertical-align: top; text-align: right;'>{UnitPriceWithoutDisc}</td>");
                sb.AppendLine($"<td style='padding: .75rem; vertical-align: top; text-align: right;'>{unitTotalStr}");
                if(discountPercentage != 0 || discount!=0)
                    sb.AppendLine($"<br><div class='discount'>You save: {DiscountTotalSum}</div></td>");
                else
                    sb.AppendLine($"</td>");
                ...
                //    sb.AppendLine("</tr>");
                //    sb.AppendLine($"<tr>");
                //    sb.AppendLine($"<td></td><td></td><td></td><td style='padding: .75rem; vertical-align: top; text-align: right;'><br>/td>");
                //}

                sb.AppendLine("</tr>");

                var taxCustomerCurrency = _currencyService.ConvertCurrency((orderItem.UnitPriceInclTax - orderItem.UnitPriceExclTax) * orderItem.Quantity, order.CurrencyRate);
                string taxStr = _priceFormatter.FormatPrice(taxCustomerCurrency, true, order.CustomerCurrencyCode, language, true);

                sb.AppendLine($"<tr>");
                sb.AppendLine($"<td></td><td></td><td></td><td></td>");
                //sb.AppendLine($"<td style='padding: .75rem; vertical-align: top; text-align: right;'>{_localizationService.GetResource("Messages.Order.Tax", languageId)} {taxStr} </td>");
                sb.AppendLine($"</tr>");
            }
                sb.AppendLine("</table>");
            
                sb.AppendLine("<table border=\"0\" style=\"width: 25%;float:right;border:1px solid #e44436;margin-top:20px;border-radius:5px;margin-bottom:20px;padding: 20px;\">");
            //WriteTotals(order, language, sb);
            //totals
            if (isfreightQuote)
                NewWriteTotalsInvoiceAndFreigth(order, language, sb, cart);
            else
                NewWriteTotals(order, language, sb, cart);
        ...
        protected virtual string NewInvoicesListToHtmlTable(List<InvoicesToPay> InvoiceApplyJson, int languageId, int vendorId, IList<ShoppingCartItem> cart = null, bool totals = true, decimal TotalPayInvoice=0)
        {
          
            var sb = new StringBuilder();
            var table = InvoiceApplyJson.ToList();
            decimal totalTotal = 0; 
            for (var i = 0; i <= table.Count - 1; i++)
            {
                var orderItem = table[i];
                var invoice = _invoiceService.GetInvoiceById(Convert.ToInt32(orderItem.OrderId));

                decimal InvoiceDiscount = Convert.ToDecimal(invoice.Total) - Convert.ToDecimal(invoice.AmountDue) - Convert.ToDecimal(orderItem.ValuePay);
                ...
                sb.AppendLine($"<tr>");
                sb.AppendLine($"<td style='padding: .75rem; vertical-align: top;'>{invoice.InvoiceNo}</td>");
                sb.AppendLine($"<td style='padding: .75rem; vertical-align: top; text-align: right;'>{orderItem.ValuePay}</td>");
               
                sb.AppendLine("</tr>");
                if (!string.IsNullOrEmpty(orderItem.ValuePay))
                    totalTotal += Convert.ToDecimal(orderItem.ValuePay);
            }
            ...
            sb.AppendLine($"<tr>");
            sb.AppendLine($"<td style='padding: .75rem; vertical-align: top; text-align: right;'>{_localizationService.GetResource("Messages.Order.Total")}</td>");
            sb.AppendLine($"<td  style='text-align: right;'>{totalTotal} </td>");
            sb.AppendLine($"</tr>");
        ...
        protected virtual string NewCreditPaymentInvoicesListToHtmlTable(List<CustomerAccountCredit> InvoiceApplyJson, int languageId, int vendorId, IList<ShoppingCartItem> cart = null, bool totals = true, decimal TotalPayInvoice = 0)
        {

            var sb = new StringBuilder();
            var table = InvoiceApplyJson.ToList();

            var totalTotalCredit = 0;
            ...
            for (var i = 0; i <= table.Count - 1; i++)
            {
                var orderItem = table[i];
                    ...
                    sb.AppendLine($"<tr>");
                    sb.AppendLine($"<td style='padding: .75rem; vertical-align: top;'>{orderItem.Transid}</td>");
                    sb.AppendLine($"<td style='padding: .75rem; vertical-align: top; text-align: right;'>{orderItem.TotalApply}</td>");
        ...
        protected virtual void NewWriteTotals(Order order, Language language, StringBuilder sb, IList<ShoppingCartItem> cart = null)
        {
            //subtotal
            string cusSubTotal;
            ...
            else
            {
                ...
                else
                {
                    if (cart != null)
                    {
                        ...
                        var taxNewRates = new SortedDictionary<decimal, decimal>();
                        ...
                        else
                        {
                            ...
                            var orderTaxInCustomerCurrency = _currencyService.ConvertCurrency(shoppingCartTax, order.CurrencyRate);
                    ...
                    {
                        var taxStr = _priceFormatter.FormatPrice(order.OrderTax, true, order.CustomerCurrencyCode, false, language);
                        cusTaxTotal = taxStr;
                    }
                }
            }
            ...
            {
                var shoppingCartTotal = _currencyService.ConvertFromPrimaryStoreCurrency(order.OrderTotal, _workContext.WorkingCurrency);
                cusTotal = _priceFormatter.FormatPrice(shoppingCartTotal, true, false);
            }

            // set Subtotal
            sb.AppendLine($"<tr>");
            sb.AppendLine($"<td style='padding: .75rem; vertical-align: top; text-align: right;'>{_localizationService.GetResource("Messages.Order.Subtotal", language.Id)}</td>");
            sb.AppendLine($"<td  style='text-align: right;'>{cusSubTotal} </td>");
            sb.AppendLine($"</tr>");
            ...
            if (displayShipping)
            {
                // Set Shipping
                sb.AppendLine($"<tr>");
                sb.AppendLine($"<td style='padding: .75rem; vertical-align: top; text-align: right;'>{_localizationService.GetResource("Messages.Order.Shipping", language.Id)}</td>");
                sb.AppendLine($"<td  style='text-align: right;'>{cusShipTotal} </td>");
                sb.AppendLine($"</tr>");
            }
            ...
            if (displayDiscount)
            {
                // Set Discount
                sb.AppendLine($"<tr>");
                sb.AppendLine($"<td style='padding: .75rem; vertical-align: top; text-align: right;'>{_localizationService.GetResource("Messages.Order.Subtotaldiscount", language.Id)}</td>");
                sb.AppendLine($"<td  style='text-align: right;'>{cusDiscount} </td>");
                sb.AppendLine($"</tr>");
            }
            ...
            // Set Tax
            sb.AppendLine($"<tr>");
            sb.AppendLine($"<td style='padding: .75rem; vertical-align: top; text-align: right;'>{_localizationService.GetResource("Messages.Order.Taxes", language.Id)}</td>");
            sb.AppendLine($"<td style='text-align: right;'>{cusTaxTotal} </td>");
            sb.AppendLine($"</tr>");

            // Set Total
            sb.AppendLine($"<tr>");
            //sb.AppendLine($"<th style='text-align: left;color: #8857d4;border-color: #9f80cf;vertical-align: bottom; padding: .75rem;'></th>");
            //sb.AppendLine($"<th style='text-align: left;color: #8857d4;border-color: #9f80cf;vertical-align: bottom;  padding: .75rem;'></th>");
            sb.AppendLine($"<th style='text-align: right;color: #263238;border-color: #263238;vertical-align: bottom;  padding: .75rem; '>{_localizationService.GetResource("Messages.Order.Ordertotal", language.Id)} </th>");
            sb.AppendLine($"<th style='color:#263238;border-color:#263238;vertical-align:bottom;padding:.75rem;padding-right: 0;text-align: right;'>{cusTotal} </th>");
            sb.AppendLine($"</tr>");

        }

        protected virtual void NewWriteTotalsInvoiceAndFreigth(Order order, Language language, StringBuilder sb, IList<ShoppingCartItem> cart = null)
        {
            //subtotal
            string cusSubTotal;
            var cusTotal = decimal.Zero;
            decimal CalculateTotal = decimal.Zero;
            decimal orderSubTotalDiscountAmount = 0;
            decimal subTotalWithoutDiscountBase = 0;
            decimal ordertotalWithDiscount = 0;
            List<Discounts.DiscountForCaching> appliedDiscounts = new List<Discounts.DiscountForCaching>();


            _orderTotalCalculationService.GetShoppingCartSubTotal(cart,false,out orderSubTotalDiscountAmount, out appliedDiscounts, out subTotalWithoutDiscountBase
                , out ordertotalWithDiscount);
            decimal subtotal = 0;

			foreach (var item in cart)
			{
                 subtotal += _priceCalculationService.GetSubTotal(item, false);

            }


            cusTotal = subtotal - subTotalWithoutDiscountBase;
            string disct = "";
            disct = _priceFormatter.FormatPrice(cusTotal, true, order.CustomerCurrencyCode, false, language);
            var subtotalString = _priceFormatter.FormatPrice(subtotal, true, order.CustomerCurrencyCode, false, language);
            var subTotalWithoutDiscountBaseString = _priceFormatter.FormatPrice(subTotalWithoutDiscountBase, true, order.CustomerCurrencyCode, false, language);

            // set Subtotal
            sb.AppendLine($"<tr>");
            sb.AppendLine($"<td style='padding: .75rem; vertical-align: top; text-align: right;'>{_localizationService.GetResource("Messages.Order.Subtotal", language.Id)}</td>");
            sb.AppendLine($"<td  style='text-align: right;'>{subtotalString} </td>");
            sb.AppendLine($"</tr>");
            ...
            if (cusTotal!=0)
            {
                ...
                sb.AppendLine($"<tr>");
                sb.AppendLine($"<td style='padding: .75rem; vertical-align: top; text-align: right;'>{_localizationService.GetResource("Messages.Order.Subtotaldiscount", language.Id)}</td>");
                sb.AppendLine($"<td  style='text-align: right;'>{disct} </td>");
                sb.AppendLine($"</tr>");

                // Set Discount
                sb.AppendLine($"<tr>");
                sb.AppendLine($"<td style='padding: .75rem; vertical-align: top; text-align: right;'>{_localizationService.GetResource("Messages.Order.Ordertotal", language.Id)}</td>");
                sb.AppendLine($"<td  style='text-align: right;'> {subTotalWithoutDiscountBaseString} </td>");
                sb.AppendLine($"</tr>");
            }
        ...
        protected virtual string GetDeliveryDateEstimate(int orderId)
        {
            // Shipping Status
            var shipments = _shipmentService.GetShipmentByOrderId(orderId);
            ...
            if (shipments != null && shipments.Count > 0)
            {
                ...
                if (shipmentTracker != null)
                {
                    ...
                    {
                        return DateTime.ParseExact(deliveryDetails.Date, "yyyyMMdd", CultureInfo.InvariantCulture).ToString("D");
                    }
                }
            }
        ...
        public virtual void AddOrderTokens(IList<Token> tokens, Order order, int languageId, int vendorId = 0, IList<ShoppingCartItem> cart = null, bool totals=true, bool showShipping = false, string ResidentialAddress = null, string TractorAccessible = null, string trailerEnterExit = null,
            string StandardDock = null, string AppointmentDelivery = null, string StandardReceiving = null, string email = null, string phoneNumber = null, bool isfreightQuote = false)
        {
            ...
            tokens.Add(new Token("Order.BillingPhoneNumber", phoneNumber));
            ...
            tokens.Add(new Token("Order.ShippingStatus", order.ShippingStatus));
            tokens.Add(new Token("Order.PickupInStore", order.PickupInStore));
            if (order.ShippingAddress == null || order.ShippingAddress?.Id == order.BillingAddress?.Id)
            {
                if (showShipping)
                {
                   tokens.Add(new Token("Order.ShippingFirstName", order.BillingAddress.FirstName));
                   tokens.Add(new Token("Order.ShippingLastName", order.BillingAddress.LastName));
                   tokens.Add(new Token("Order.ShippingPhoneNumber", order.BillingAddress.PhoneNumber));
                   tokens.Add(new Token("Order.ShippingEmail", order.BillingAddress.Email));
                   tokens.Add(new Token("Order.ShippingFaxNumber", order.BillingAddress.FaxNumber));
                   tokens.Add(new Token("Order.ShippingCompany", order.BillingAddress.Company));
                   tokens.Add(new Token("Order.ShippingAddress1", order.BillingAddress.Address1));
                   tokens.Add(new Token("Order.ShippingAddress2", order.BillingAddress.Address2));
                   tokens.Add(new Token("Order.ShippingCity", order.BillingAddress.City));
                   tokens.Add(new Token("Order.ShippingCounty", order.BillingAddress.County));
                   tokens.Add(new Token("Order.ShippingStateProvince", order.BillingAddress.StateProvince != null ? _localizationService.GetLocalized(order.BillingAddress.StateProvince, x => x.Name) : string.Empty));
                   tokens.Add(new Token("Order.ShippingZipPostalCode", order.BillingAddress.ZipPostalCode));
                   tokens.Add(new Token("Order.ShippingCountry", order.BillingAddress.Country != null ? _localizationService.GetLocalized(order.BillingAddress.Country, x => x.Name) : string.Empty));
                   tokens.Add(new Token("Order.ShippingCustomAttributes", _addressAttributeFormatter.FormatAttributes(order.BillingAddress.CustomAttributes), true));
                }else{
                    tokens.Add(new Token("Order.Pickup", order.ShippingMethod));
                    //tokens.Add(new Token("Order.ShippingLastName", string.Empty));
                    //tokens.Add(new Token("Order.ShippingPhoneNumber", string.Empty));
                    //tokens.Add(new Token("Order.ShippingEmail", string.Empty));
                    //tokens.Add(new Token("Order.ShippingFaxNumber", string.Empty));
                    //tokens.Add(new Token("Order.ShippingCompany", string.Empty));
                    //tokens.Add(new Token("Order.ShippingAddress1", string.Empty));
                    //tokens.Add(new Token("Order.ShippingAddress2", string.Empty));
                    //tokens.Add(new Token("Order.ShippingCity", string.Empty));
                    //tokens.Add(new Token("Order.ShippingCounty", string.Empty));
                    //tokens.Add(new Token("Order.ShippingStateProvince", string.Empty));
                    //tokens.Add(new Token("Order.ShippingZipPostalCode", string.Empty));
                    //tokens.Add(new Token("Order.ShippingCountry", string.Empty));
                    //tokens.Add(new Token("Order.ShippingCustomAttributes", string.Empty));
                }
            }
            else
            {
            ...
            tokens.Add(new Token("Order.Product(s)", NewProductListToHtmlTable(order, languageId, vendorId, cart, totals, isfreightQuote), true));
            

            order.CreatedOnUtc = order.CreatedOnUtc == DateTime.MinValue ? DateTime.Now : order.CreatedOnUtc;
            if (language != null && !string.IsNullOrEmpty(language.LanguageCulture) && order.Customer != null)
            {
            ...
            else
            {
                ...
                {
                    string ResidentialAddressInfo = string.Format("Residential Address?: {1}{0}",Environment.NewLine, ResidentialAddress);
                    tokens.Add(new Token("Order.ResidentialAddress", ResidentialAddressInfo));
                }

                if (TractorAccessible != null)
                {
                    string TractorAccessibleInfo = string.Format("Tractor Trailer Accessible?: {1}{0}", Environment.NewLine, TractorAccessible);
                    tokens.Add(new Token("Order.TractorAccessible", TractorAccessibleInfo));
                }

                if (trailerEnterExit != null)
                {
                    string trailerEnterExitInfo = string.Format("A 53ft trailer can enter and exit the property safely?: {1}{0}", Environment.NewLine, trailerEnterExit);
                    tokens.Add(new Token("Order.trailerEnterExit", trailerEnterExitInfo));
                }

                if (StandardDock != null)
                {
                    string StandardDockInfo= string.Format("Standard Height Loading Dock or Forklift Available?: {1}{0}", Environment.NewLine, StandardDock);
                    tokens.Add(new Token("Order.StandardDock", StandardDockInfo));
                }
                if (AppointmentDelivery != null)
                {
                    string AppointmentDeliveryinfo = string.Format("Appointment Needed for Delivery?: {1}{0}", Environment.NewLine, AppointmentDelivery);
                    tokens.Add(new Token("Order.AppointmentDelivery", AppointmentDeliveryinfo));
                }
                if (StandardReceiving != null)
                {
                    string StandardReceivingInfo = string.Format("Standard Receiving Hours (9am-4pm): {1}{0}", Environment.NewLine, StandardReceiving);
                    tokens.Add(new Token("Order.StandardReceiving", StandardReceivingInfo));
                }


            }

            var customerInfoUrl = RouteUrl(order.StoreId, "CustomerInfo", null);
            tokens.Add(new Token("Order.CustomerInfo", customerInfoUrl, true));
            ...
            tokens.Add(new Token("Order.Orderstatus", order.OrderStatus != 0 ? _localizationService.GetLocalizedEnum(order.OrderStatus) : string.Empty));

            tokens.Add(new Token("Order.DeliveryDateEstimate", GetDeliveryDateEstimate(order.Id)));
        ...
        public virtual void AddCustomerTokens(IList<Token> tokens, Customer customer)
        {
            ...
            tokens.Add(new Token("Order.CustomerEmail", customer.Email)); //add param token 
            tokens.Add(new Token("Customer.Username", customer.Username));

            var company = customer.Companies.FirstOrDefault();
            tokens.Add(new Token("Order.BillingCompany", company != null ? company.CompanyName : string.Empty));
            ...
            var passwordCreateConfirm = RouteUrl(routeName: "PasswordCreateConfirm", routeValues: new { token = customer.CustomerGuid.ToString(), email = customer.Email });
            ...
            var homeUrl = RouteUrl(routeName: "Homepage", routeValues: null);
            var dealMonthUrl = GetPromotionProductUrl();
            var contactUsUrl = RouteUrl(routeName: "ContactUs", routeValues: null);

            tokens.Add(new Token("Customer.PasswordCreateURL", passwordCreateConfirm, true));
            tokens.Add(new Token("Customer.PasswordRecoveryURL", passwordRecoveryUrl, true));
            tokens.Add(new Token("Customer.AccountActivationURL", accountActivationUrl, true));
            tokens.Add(new Token("Customer.EmailRevalidationURL", emailRevalidationUrl, true));
            tokens.Add(new Token("Wishlist.URLForCustomer", wishlistUrl, true));

            tokens.Add(new Token("HomeURL", homeUrl, true));
            tokens.Add(new Token("DealMonthUrl", dealMonthUrl, true));
            tokens.Add(new Token("ContactUsUrl", contactUsUrl, true));

            //event notification
            _eventPublisher.EntityTokensAdded(customer, tokens);
        }

        private string GetPromotionProductUrl()
        {
            //Promotion banner
            ...
            var dealMonthUrl = RouteUrl(routeName: "Homepage", routeValues: null);
            return dealMonthUrl;
        }
        ...
        /// <param name="tokens">List of already added tokens</param>
        /// <param name="customer">Customer</param>
        public virtual void AddAccountCustomerTokens(IList<Token> tokens, Customer customer)
        {
            tokens.Add(new Token("Customer.Email", customer.Email));
            tokens.Add(new Token("Customer.Username", customer.Username));
        ...
        #region Invoice
        public virtual void AddInvoiceTokens(IList<Token> tokens, List<Services.Payments.InvoicesToPay> InvoiceApplyJson, List<CustomerAccountCredit> CustomerAccountCredit, decimal TotalPayInvoice, Address BillingAddress, int languageId, int vendorId = 0, IList<ShoppingCartItem> cart = null, bool totals = true, bool showShipping = false, string ResidentialAddress = null, string TractorAccessible = null, string trailerEnterExit = null,
            string StandardDock = null, string AppointmentDelivery = null, string StandardReceiving = null)
        {
            var customerInfoUrl = RouteUrl(_storeContext.CurrentStore.Id, "CustomerInfo", null);
            tokens.Add(new Token("Order.CustomerInfo", customerInfoUrl, true));
            ...
            tokens.Add(new Token("Order.invoice(s)", NewInvoicesListToHtmlTable(InvoiceApplyJson, languageId, vendorId, cart, totals, TotalPayInvoice), true)); tokens.Add(new Token("Order.invoice(s)", NewInvoicesListToHtmlTable(InvoiceApplyJson, languageId, vendorId, cart, totals, TotalPayInvoice), true));
            tokens.Add(new Token("Order.Credits(s)", NewCreditPaymentInvoicesListToHtmlTable(CustomerAccountCredit, languageId, vendorId, cart, totals, TotalPayInvoice), true));
            ...
            tokens.Add(new Token("Order.BillingStateProvince", BillingAddress?.StateProvince != null ? _localizationService.GetLocalized(BillingAddress?.StateProvince, x => x.Name) : string.Empty));
            tokens.Add(new Token("Order.BillingZipPostalCode", BillingAddress?.ZipPostalCode));
            tokens.Add(new Token("Order.BillingCountry", BillingAddress?.Country != null ? _localizationService.GetLocalized(BillingAddress?.Country, x => x.Name) : string.Empty));
            ...
            //var paymentMethod = _paymentPluginManager.LoadPluginBySystemName(order.PaymentMethodSystemName);
           // var paymentMethodName = paymentMethod != null ? _localizationService.GetLocalizedFriendlyName(paymentMethod, _workContext.WorkingLanguage.Id) : order.PaymentMethodSystemName;
            tokens.Add(new Token("Order.PaymentMethod", "Payments.AuthorizeNet"));
          
            var sbCustomValues = new StringBuilder();
            
            //var language = _languageService.GetLanguageById(languageId);
            //order.LastModifiedDate = order.LastModifiedDate == DateTime.MinValue ? DateTime.Now : order.LastModifiedDate;
