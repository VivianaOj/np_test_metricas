...
namespace Nop.Services.Messages
{
    ...
    public partial class WorkflowMessageService : IWorkflowMessageService
    {
        ...
        public WorkflowMessageService(CommonSettings commonSettings,
            ...
            IGenericAttributeService genericAttributeService, ISettingService settingService)
        {
        ...
        /// <param name="languageId">Message language identifier</param>
        /// <returns>Queued email identifier</returns>
        public virtual IList<int> SendCustomerCreatePasswordMessage(Customer customer, int languageId)
        {
            if (customer == null)
                throw new ArgumentNullException(nameof(customer));
            ...
            var messageTemplates = GetActiveMessageTemplates(MessageTemplateSystemNames.CustomerPasswordCreateMessage, store.Id);
            if (!messageTemplates.Any())
                return new List<int>();
        ...
        /// <summary>
        /// Send shopping cart awaiting message to a customer
        /// </summary>
        /// <param name="customer"></param>
        /// <param name="languageId"></param>
        /// <returns></returns>
        public virtual IList<int> SendCustomerShoppingCartAwaitingMessage(Customer customer, int languageId)
        {
            if (customer == null)
                throw new ArgumentNullException(nameof(customer));
            ...
            var messageTemplates = GetActiveMessageTemplates(MessageTemplateSystemNames.ShoppingCartAwaitingCheckout, store.Id);
            if (!messageTemplates.Any())
                return new List<int>();
            ...
            if (customer.BillingAddressId != null)
                order.BillingAddressId = Convert.ToInt32(customer.BillingAddressId);
            else
                order.BillingAddress = customer.BillingAddress;

            if (order.BillingAddress != null)
            {
                order.BillingAddress.PhoneNumber = _genericAttributeService.GetAttribute<string>(customer, NopCustomerDefaults.PhoneAttribute);
            ...
            //Array.FindAll(customer.ShoppingCartItems, i => i.s);

            foreach (var item in customer.ShoppingCartItems)
            {
                if (item.ShoppingCartTypeId == 1) //verify if shoppingcart item or wishlist
                {
            ...
            _messageTokenProvider.AddOrderTokens(commonTokens, order, languageId, 0, null, false);
            _messageTokenProvider.AddAccountCustomerTokens(commonTokens, customer);
        ...
        /// <param name="languageId"></param>
        /// <returns></returns>
        public virtual IList<int> SendUpdateOrderNotification(Order order, int languageId)
        {
            if (order == null)
            ...
            var messageTemplates = GetActiveMessageTemplates(MessageTemplateSystemNames.OrderUpdateCustomerNotification, store.Id);
            if (!messageTemplates.Any())
                return new List<int>();
            ...
            return messageTemplates.Select(messageTemplate =>
            {
                ...
                #region send a copy
                var toEmailcopy = order.Customer.Email;
                var toNamecopy = $"{order.Customer.Username}";
                if (toEmailcopy != null)
                {
                    if (!toEmailcopy.Equals(order.BillingAddress.Email))
                    {
                        SendNotification(messageTemplate, emailAccount, languageId, tokens, toEmailcopy, toNamecopy);
                    }
                }
                #endregion
        ...
        public virtual IList<int> SendOrderCancelledCustomerNotification(Order order, int languageId)
        {
            ...
            return messageTemplates.Select(messageTemplate =>
            {
                ...
                #region
                var toEmailCopy = order.BillingAddress.Email;
                var toNameCopy = $"{order.BillingAddress.FirstName} {order.BillingAddress.LastName}";
                SendNotification(messageTemplate, emailAccount, languageId, tokens, toEmailCopy, toNameCopy);
                #endregion

                var toEmail = _workContext.CurrentCustomer.Email;
                var toName = $"{_workContext.CurrentCustomer.Username}";
                return SendNotification(messageTemplate, emailAccount, languageId, tokens, toEmail, toName);
            }).ToList();
        }
        ...
        public virtual IList<int> SendContactUsMessage(int languageId, string senderEmail,
            string senderName, string subject, string body, string phoneNumber, string company)
        {
            var alternativeEmail = _settingService.GetSetting("commonsettings.emailcontactus.alternative").Value;
            ...
            return messageTemplates.Select(messageTemplate =>
            {
                ...
                if (_commonSettings.UseSystemEmailForContactUsForm)
                {
                    ...
                    body = $"<strong>From</strong>: {WebUtility.HtmlEncode(senderName)} - {WebUtility.HtmlEncode(senderEmail)} - {WebUtility.HtmlEncode(phoneNumber)}, {WebUtility.HtmlEncode(company)} <br /><br />{body}";
                }
                else
                {
        ...
        public virtual IList<int> SendEmailFreightQuote(Order order, IList<ShoppingCartItem> cart, int languageId, string ResidentialAddress, string TractorAccessible, string trailerEnterExit,
            string StandardDock, string AppointmentDelivery, string StandardReceiving, string email, string phoneNumber)
        {
            var store = _storeContext.CurrentStore;
            languageId = EnsureLanguageIsActive(languageId, store.Id);

            var messageTemplates = GetActiveMessageTemplates(MessageTemplateSystemNames.ShipmentSentFreightQuoteNotification, store.Id);
            if (!messageTemplates.Any())
                return new List<int>();

            var messageCustomerTemplates = GetActiveMessageTemplates(MessageTemplateSystemNames.ShipmentSentFreightQuoteCustomerNotification, store.Id).FirstOrDefault();
            if (messageCustomerTemplates==null)
                return new List<int>();


            //tokens
            var commonTokens = new List<Token>();
            _messageTokenProvider.AddOrderTokens(commonTokens, order, languageId, cart: cart, totals: false, showShipping: true, ResidentialAddress: ResidentialAddress, TractorAccessible: TractorAccessible, trailerEnterExit: trailerEnterExit,
            StandardDock: StandardDock, AppointmentDelivery: AppointmentDelivery, StandardReceiving: StandardReceiving, email:email, phoneNumber:phoneNumber,  isfreightQuote :  true);
            ...
            return messageTemplates.Select(messageTemplate =>
            {
                ...
                if (order.Customer!=null)
                {
                    ...
                    if (toEmailcopy != null)
                    {
                            SendNotification(messageCustomerTemplates, emailAccount, languageId, tokens, toEmailcopy, toNamecopy);
                ...
                {
                    var customerAccount = _customerService.GetCustomerById(_workContext.CurrentCustomer.Id);
                    var toEmailAccount = _workContext.CurrentCustomer.Username;
                    var toNameAccount = _customerService.GetCustomerFullName(customerAccount);
                    SendNotification(messageTemplate, emailAccount, languageId, tokens, toEmailAccount, toNameAccount);
                }
                */
                var toEmailcopy = order.BillingAddress.Email;
                var toEmailAccount = _workContext.CurrentCustomer.Username;
                ...
                if (_workContext.CurrentCustomer.Username != null)
                {
                    var customerAccount = _customerService.GetCustomerById(_workContext.CurrentCustomer.Id);
                    var toNameAccount = _customerService.GetCustomerFullName(customerAccount);
                    SendNotification(messageTemplate, emailAccount, languageId, tokens, toEmailAccount, toNameAccount);
        ...
        public virtual IList<int> SendEmailOrderCompletedWasPaid(Order order, int languageId,
            string attachmentFilePath = null, string attachmentFileName = null)
        {
            if (order == null)
                throw new ArgumentNullException(nameof(order));
            ...
            var messageTemplates = GetActiveMessageTemplates(MessageTemplateSystemNames.OrderCompletedWasPaid, store.Id);
            if (!messageTemplates.Any())
                return new List<int>();
        ...
        #region Invoice
        public virtual IList<int> SendInvoicePaidCustomerNotification(List<Services.Payments.InvoicesToPay> order, decimal TotalPayInvoice,  Address address, int languageId,
           string attachmentFilePath = null, string attachmentFileName = null, List<CustomerAccountCredit> CustomerAccountCredit=null)
        
        {
            if (order == null)
                throw new ArgumentNullException(nameof(order));

            var alternativeEmail = _settingService.GetSetting("commonsettings.payinvoice.email").Value;

            var store = _storeContext.CurrentStore;
            languageId = EnsureLanguageIsActive(languageId, store.Id);

            var messageTemplates = GetActiveMessageTemplates(MessageTemplateSystemNames.InvoiceCompletedWasPaidNotification, store.Id);
            if (!messageTemplates.Any())
                return new List<int>();

            //tokens
            var commonTokens = new List<Token>();
            _messageTokenProvider.AddInvoiceTokens(commonTokens, order, CustomerAccountCredit, TotalPayInvoice, address,  languageId);
            _messageTokenProvider.AddCustomerTokens(commonTokens, _workContext.CurrentCustomer);
            ...
            return messageTemplates.Select(messageTemplate =>
            {
                ...
                //#region send a copy
                //var toEmailcopy = _workContext.CurrentCustomer.Email;
                //var toNamecopy = $"{_workContext.CurrentCustomer.Username}";
                //if (toEmailcopy != null)
                //{
                //    if (!toEmailcopy.Equals(_workContext.CurrentCustomer.BillingAddress.Email))
                //    {
                //        SendNotification(messageTemplate, emailAccount, languageId, tokens, toEmailcopy, toNamecopy,
                //        attachmentFilePath, attachmentFileName);
                //    }
