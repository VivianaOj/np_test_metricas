...
namespace Nop.Services.Customers
{
    ...
    public partial class CustomerService : ICustomerService
    {
        ...
        public CustomerService(CustomerSettings customerSettings,
            ...
            IRepository<CustomerAddressMapping> customerAddress,
            ...
            int pageIndex = 0, int pageSize = int.MaxValue, bool getOnlyTotalCount = false)
        {
            ...
            if (query.Count() > 0)
                query = query.Where(r => r.LastLoginDateUtc != null);

            var customers = new PagedList<Customer>(query, pageIndex, pageSize, getOnlyTotalCount);
            return customers;
        }

        public virtual IPagedList<Customer> GetAllReportCustomers(DateTime? createdFromUtc = null, DateTime? createdToUtc = null,
         int[]  customerRoleIds = null, int pageIndex = 0, int pageSize = int.MaxValue, bool getOnlyTotalCount = false, bool isAccountCustomer = false)
        {
            var query = _customerRepository.Table;
            ...
            {
                query = query.Join(_customerPasswordRepository.Table, x => x.Id, y => y.CustomerId,
                        (x, y) => new { Customer = x, Mapping = y })
                    .Where(z => z.Customer.Id == z.Mapping.CustomerId && z.Customer.NetsuitId != 0)
                    .Select(z => z.Customer)
                    .Distinct();
            }
            else
        ...
        public virtual IPagedList<Customer> GetCustomersWithShoppingCarts(ShoppingCartType? shoppingCartType = null,
            int storeId = 0, int? productId = null, DateTime? createdFromUtc = null, DateTime? createdToUtc = null, int? countryId = null, int[] SelectedCustomerRoleIds=null,
            int pageIndex = 0, int pageSize = int.MaxValue, DateTime? updatedFromUtc = null, DateTime? updatedToUtc = null)
        {
            ...
            //    items = items.Where(item => updatedFromUtc.Value >= item.UpdatedOnUtc);

            ////filter shopping cart items by date
            //if (updatedFromUtc.HasValue)
            //    items = items.Where(item => updatedFromUtc.Value <= item.UpdatedOnUtc);
            //if (updatedToUtc.HasValue)
            //    items = items.Where(item => updatedToUtc.Value >= item.UpdatedOnUtc);
            ...
            {
                if (SelectedCustomerRoleIds != null && SelectedCustomerRoleIds.Length > 0)
                    customers = customers.Where(c => c.CustomerCustomerRoleMappings.Select(mapping => mapping.CustomerRoleId).Intersect(SelectedCustomerRoleIds).Any());
            }

            //var itemCustomer = items.GroupBy(r => r.Customer);
            ...
            ////get customers with shopping carts
            //var customersWithCarts = items.GroupBy(item => item.Customer)
            //    .Select(cart => new { CustomerId = cart.Key, CreatedOnUtc = cart.Max(item => item.CreatedOnUtc) })
            //    .OrderByDescending(cart => cart.CreatedOnUtc)
            //    .Select(cart => cart.CustomerId)
            //    .ToList() //currently GroupBy and Join (when used together) are incorrectly translated from LINQ to SQL, see https://github.com/aspnet/EntityFrameworkCore/issues/12826, so we continue execution on the server side
        ...
        public virtual IList<Customer> GetCustomersByIds(int[] customerIds)
        {
            ...
            var query = from c in _customerRepository.Table
                        where customerIds.Contains(c.Id) && !c.Deleted && c.NetsuitId!=0
                        select c;
        ...
        /// <summary>
        /// Get customer by customerGuid and email
        /// </summary>
        /// <param name="customerGuid"></param>
        /// <param name="email"></param>
        /// <returns></returns>
        public virtual Customer GetCustomerByGuidEmail(Guid customerGuid, string email)
        {
            if (string.IsNullOrWhiteSpace(email))
                return null;

            if (string.IsNullOrWhiteSpace(customerGuid.ToString()))
                return null;

            var query = from c in _customerRepository.Table
                        orderby c.Id
                        where c.Email == email && c.CustomerGuid == customerGuid
                        select c;
            var customer = query.FirstOrDefault();
            return customer;
        }
        ...
        public virtual Address GetCustomerAddress(int customerId, int addressId)
        {
            ...
            var query = from address in _customerAddressRepository.Table
                        join cam in _customerAddress.Table on address.Id equals cam.AddressId
        ...
        public Customer GetCustomerByNetsuitId(int NetsuitId, string email=null)
        {
            if ( NetsuitId == 0)
                throw new ArgumentNullException(nameof(NetsuitId));
            //bool flag = false;
            //List<Customer> list = new List<Customer>();

            var query = _customerRepository.Table.Where(p => !p.Deleted && p.NetsuitId == NetsuitId);
            ...
            //        Email = item.Email,
            //        EmailToRevalidate = item.EmailToRevalidate,
            //        AdminComment = item.AdminComment,
            //        IsTaxExempt = item.IsTaxExempt,
            //        AffiliateId = item.AffiliateId,
            //        VendorId = item.VendorId,
            //        HasShoppingCartItems = item.HasShoppingCartItems,
            //        RequireReLogin = item.RequireReLogin,
            //        FailedLoginAttempts = item.FailedLoginAttempts,
            //        CannotLoginUntilDateUtc = item.CannotLoginUntilDateUtc,
            //        Active = item.Active,
            ...
            //        LastLoginDateUtc = item.LastLoginDateUtc,
            //        LastActivityDateUtc = item.LastActivityDateUtc,
            //        RegisteredInStoreId = item.RegisteredInStoreId,
            //        BillingAddressId = item.BillingAddressId,
            //        ShippingAddressId = item.ShippingAddressId,
            //        Parent = item.Parent,
            //        NetsuitId = item.NetsuitId
            //    });
        ...
        public List<Customer> GetChildsByNetsuitId(int NetsuitId)
        {
            if (NetsuitId == null)
                throw new ArgumentNullException(nameof(NetsuitId));

            var query = _customerRepository.Table.Where(x => x.Parent == NetsuitId).ToList();
            return query;
        }

        public int SearchCustomerRole(int CustomerId)
        {
            if (CustomerId == 0)
                throw new ArgumentNullException(nameof(CustomerId));

            CustomerCustomerRoleMapping Model = new CustomerCustomerRoleMapping();
            var query = _customerCustomerRoleMappingRepository.Table.Where(x => x.CustomerId == CustomerId).FirstOrDefault();

            Model.CustomerId = query.CustomerId;
            Model.CustomerRoleId = query.CustomerRoleId;

            return Model.CustomerRoleId;
        ...
        public virtual List<CustomerCustomerRoleMapping> GetCustomerCustomerOrleById(int CustomerId)
        {
            if (CustomerId == 0)
                return null;

            var query = _customerCustomerRoleMappingRepository.Table.Where(r => r.CustomerId == CustomerId).ToList();
            return query;
        }
