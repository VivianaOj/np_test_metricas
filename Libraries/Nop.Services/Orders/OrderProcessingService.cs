...
namespace Nop.Services.Orders
{
    ...
    public partial class OrderProcessingService : IOrderProcessingService
    {
        ...
        protected class PlaceOrderContainer
        {
            ...
            public string ShippingNNDeliverySend { get; set; }
            public int IdShippingNNDeliverySend { get; set; }
        ...
        protected virtual PlaceOrderContainer PreparePlaceOrderDetails(ProcessPaymentRequest processPaymentRequest)
        {
            ...
            details.BillingAddress.Residential = details.Customer.BillingAddress.Residential;
            ...
            if (_shoppingCartService.ShoppingCartRequiresShipping(details.Cart))
            {
                ...
                else
                {
                    ...
                    details.ShippingAddress.Residential = details.Customer.ShippingAddress.Residential;
                ...
                if (shippingOption != null)
                {
                    ...
                    details.ShippingNNDeliverySend = shippingOption.ValueToSendOrder;
                    details.IdShippingNNDeliverySend = shippingOption.IdValueToSendOrder;
                    details.PickupPersonNote = shippingOption.PickupPersonNote;
                }
            ...
            //tax amount
            details.OrderTaxTotal = _orderTotalCalculationService.GetTaxTotal(details.Cart, shippingRateComputationMethods, out var taxRatesDictionary,false, details.OrderShippingTotalInclTax,true);
        ...
        {
            var PoNumber = _genericAttributeService.GetAttribute<string>(_workContext.CurrentCustomer, NopCustomerDefaults.PoNumberAttribute);
            
            var BillingAddress = details.BillingAddress;

            if (BillingAddress == null)
            {
                if (processPaymentRequest.paymentProfileId != null)
                {
                    //var GetAddressPaymentProfile = _customerPaymentRepository.Table.Where(x => x.CustomerId == processPaymentRequest.CustomerId && x.CustomerPaymentProfileList == processPaymentRequest.paymentProfileId && x.CustomerProfileId == processPaymentRequest.customerProfileId && x.ResultCode != "Error").FirstOrDefault();
                    //_customerPaymentService.GetPaymentProfileByProfile(processPaymentRequest.customerProfileId.ToString(), processPaymentRequest.paymentProfileId.ToString(), processPaymentRequest.CustomerId);

                    if (processPaymentRequest.AddressCustomerProfileId != null)
                    {
                        BillingAddress = _addressService.GetAddressById(processPaymentRequest.AddressCustomerProfileId);
                    }
                }
            }
            var NetsuiteLocation = "";
            var NetsuiteLocationName = _genericAttributeService.GetAttribute<ShippingOption>(details.Customer,
                              NopCustomerDefaults.SelectedShippingOptionAttributeIsCommercial, _storeContext.CurrentStore.Id);
            if (NetsuiteLocationName != null)
            {
                

                if (!string.IsNullOrEmpty(NetsuiteLocationName.NetsuiteLocationName))
                    NetsuiteLocation = NetsuiteLocationName.NetsuiteLocationName;
            }
            ...
            var order = new Order
            {
                ...
                ShippingNNDeliverySend = details.ShippingNNDeliverySend,
                IdShippingNNDeliverySend = details.IdShippingNNDeliverySend,
                PO = PoNumber,
                PickupPersonNote = details.PickupPersonNote,
                Source = "REST Web Services",
                WarehouseLocationNN = NetsuiteLocation
            };

            //var validAccountCustomer = _customerService.GetCustomerByEmail(order.BillingAddress?.Email);
            //if (validAccountCustomer.NetsuitId != 0)
            //{
            //    var getCompanyByCustomer = validAccountCustomer.CompanyCustomerMappings.Where(r => r.DefaultCompany).FirstOrDefault();

            //    if(getCompanyByCustomer!=null)
            //        order.CompanyId = validAccountCustomer.CompanyCustomerMappings.Where(r=>r.DefaultCompany).FirstOrDefault().CompanyId;
            //    else
            //        order.CompanyId = validAccountCustomer.Companies.FirstOrDefault().Id;
            //    //option 1 send first company 
            //    // option 2 if exist only 1 company select company.
            //    // if 2 or more companies send webstore.
            //}
            //else
            //{
                if (_workContext.CurrentCustomer.IsRegistered() && _workContext.CurrentCustomer.Companies.Any())
                {
                    if (_workContext.WorkingCompany != null)
                        order.CompanyId = _workContext.WorkingCompany.Id;
                }
                else
                    order.CompanyId = Convert.ToInt32(_settingService.GetSetting("WebStoreCustomer.id")?.Value);
           // }
        ...
        protected virtual void SetOrderStatus(Order order, OrderStatus os, bool notifyCustomer)
        {
                ...
                && notifyCustomer)
            {
                ...
                //var orderCompletedCustomerNotificationQueuedEmailIdsWasPaid = _workflowMessageService
                //    .SendEmailOrderCompletedWasPaid(order, order.CustomerLanguageId, orderCompletedAttachmentFilePath,
                //    orderCompletedAttachmentFileName);
        ...
        protected virtual void MoveShoppingCartItemsToOrderItems(PlaceOrderContainer details, Order order)
        {
            foreach (var sc in details.Cart)
            {
                ...
                var scUnitPriceInclTax =
                    _taxService.GetProductPrice(sc.Product, scUnitPrice, true, details.Customer, out var _, out _, false, decimal.Zero);
                var scUnitPriceExclTax =
                    _taxService.GetProductPrice(sc.Product, scUnitPrice, false, details.Customer, out _, out _, false, decimal.Zero);
                var scSubTotalInclTax =
                    _taxService.GetProductPrice(sc.Product, scSubTotal, true, details.Customer, out _, out _, false, decimal.Zero);
                var scSubTotalExclTax =
                    _taxService.GetProductPrice(sc.Product, scSubTotal, false, details.Customer, out _, out _, false, decimal.Zero);
                var discountAmountInclTax =
                    _taxService.GetProductPrice(sc.Product, discountAmount, true, details.Customer, out _, out _, false, decimal.Zero);
                var discountAmountExclTax =
                    _taxService.GetProductPrice(sc.Product, discountAmount, false, details.Customer, out _, out _, false, decimal.Zero);
                //End N&N Changes
        ...
        public virtual PlaceOrderResult PlaceOrder(ProcessPaymentRequest processPaymentRequest)
        {
            ...
            try
            {
                ...
                if (details.OrderTaxTotal == 0)
                {
                    details.OrderTaxTotal= _orderTotalCalculationService.GetTaxTotal(details.Cart,null, out var taxRatesDictionary);
                }
        ...
        /// <param name="processPaymentRequest">Process payment request</param>
        /// <returns>Place order result</returns>
        public virtual PlaceOrderResult PlaceOrderValidate(ProcessPaymentRequest processPaymentRequest)
        {
            if (processPaymentRequest == null)
                throw new ArgumentNullException(nameof(processPaymentRequest));
        ...
        public virtual PlaceOrderResult RetryPlaceOrder(ProcessPaymentRequest processPaymentRequest)
        {
            if (processPaymentRequest == null)
                throw new ArgumentNullException(nameof(processPaymentRequest));
            ...
            try
            {
                ...
                var details = new PlaceOrderContainer
                {
                    ...
                    IsRecurringShoppingCart = false,
                    Customer = _workContext.CurrentCustomer
                };
                var customerInfo = _workContext.CurrentCustomer;
                if (customerInfo.Parent != 0)
                {
                    if(customerInfo.BillingAddressId == null)
                    {
                        var addressId = _workContext.WorkingCompany.CompanyAddressMappings.FirstOrDefault(r=>r.IsBilling)?.AddressId;
                        if(addressId!=null)
                            customerInfo.BillingAddressId = addressId;

                    }
                    if (customerInfo.ShippingAddressId == null)
                    {
                        var addressId = _workContext.WorkingCompany.CompanyAddressMappings.FirstOrDefault(r => r.IsShipping)?.AddressId;
                        if (addressId != null)
                            customerInfo.ShippingAddressId = addressId;
                ...
                if (string.IsNullOrEmpty(processPaymentRequest.CreditCardNumber) && processPaymentRequest.CreditsApply != null
                     && processPaymentRequest.CustomerId==0)
                {
                ...
                if (processPaymentResult.Success)
                {
                    ...
                    if (order != null)
                    {
                        ...
                        var saveCreditCard = SaveCreditDetails(processPaymentRequest, processPaymentResult, details);
                        //notifications
                        SendNotificationsAndSaveNotes(order);
                    ...
                    {
                        //result.PlacedOrderInvoice = invoice; 
                        // var invoice = _orderService.GetInvoiceById(processPaymentRequest.invoiceId);

                        //reset checkout data
                        //_customerService.ResetCheckoutData(details.Customer, processPaymentRequest.StoreId, clearCouponCodes: true, clearCheckoutAttributes: true);
                        //_customerActivityService.InsertActivity("PublicStore.PlaceOrder",
                        //    string.Format(_localizationService.GetResource("ActivityLog.PublicStore.PlaceOrder"), invoice.Id), invoice);

                        ////check order status
                        //CheckOrderStatus(order);

                        //raise event       
                        //_eventPublisher.Publish(new OrderPlacedEvent(invoice));

                        //if (invoice.PaymentStatus == PaymentStatus.Paid)
                        //    ProcessOrderPaid(invoice);
                        //if (processPaymentRequest.IsInvoice)
                        //{
                        //    if (processPaymentRequest.invoiceId == 0)
                        //    {
                        //        //var myDeserializedClass = JsonConvert.DeserializeObject<InvoiceModel>(processPaymentRequest.CreditsApply);
                        //        //var CustomerAccountCredit = JsonConvert.DeserializeObject<Nop.Core.Domain.Invoice.CustomerAccountCredit>(processPaymentRequest.InvoicesToPay);
                        ...
                        //                //    }
                        //                //}
                        //                //invoice.CardType = processPaymentResult.AllowStoringCreditCardNumber ? _encryptionService.EncryptText(processPaymentRequest.CreditCardType) : string.Empty;
                        //                //invoice.CardName = processPaymentResult.AllowStoringCreditCardNumber ? _encryptionService.EncryptText(processPaymentRequest.CreditCardName) : string.Empty;
                        //                //invoice.CardNumber = processPaymentResult.AllowStoringCreditCardNumber ? _encryptionService.EncryptText(processPaymentRequest.CreditCardNumber) : string.Empty;
                        //                //invoice.MaskedCreditCardNumber = _encryptionService.EncryptText(_paymentService.GetMaskedCreditCardNumber(processPaymentRequest.CreditCardNumber));
                        //                //invoice.CardCvv2 = processPaymentResult.AllowStoringCreditCardNumber ? _encryptionService.EncryptText(processPaymentRequest.CreditCardCvv2) : string.Empty;
                        //                //invoice.CardExpirationMonth = processPaymentResult.AllowStoringCreditCardNumber ? _encryptionService.EncryptText(processPaymentRequest.CreditCardExpireMonth.ToString()) : string.Empty;
                        //                //invoice.CardExpirationYear = processPaymentResult.AllowStoringCreditCardNumber ? _encryptionService.EncryptText(processPaymentRequest.CreditCardExpireYear.ToString()) : string.Empty;
                        ...
                        //               // var saveCreditCard = SaveCreditDetails(processPaymentRequest, processPaymentResult, details);
                        //                //notifications
                        //                SendNotificationsAndSaveNotes(invoice);
        ...
        protected virtual Order SaveCreditDetails(ProcessPaymentRequest processPaymentRequest, ProcessPaymentResult processPaymentResult, PlaceOrderContainer details)
        {
            return null;
        }
            ...
            try
            {
                ...
                if (!skipPaymentWorkflow)
                {
                    ...
                    if (details.InitialOrder.AllowStoringCreditCardNumber)
                    {
                        ...
                        try
                        {
                            processPaymentRequest.CreditCardExpireMonth =_encryptionService.DecryptText(details.InitialOrder.CardExpirationMonth);
        ...
        public virtual void ReOrder(Order order)
        {
            ...
            foreach (var orderItem in order.OrderItems)
            {
                _shoppingCartService.AddToCart(_workContext.CurrentCustomer, orderItem.Product,
