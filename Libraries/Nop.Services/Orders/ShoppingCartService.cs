...
namespace Nop.Services.Orders
{
    ...
    public partial class ShoppingCartService : IShoppingCartService
    {
        ...
        /// <summary>
        /// Get  expired shopping cart items
        /// </summary>
        /// <param name="olderThanUtc">Older than date and time</param>
        /// <returns>Number of items items</returns>
        public virtual int GetExpiredShoppingCartItems(DateTime olderThanUtc, DateTime endDate)
        {
            var query = from sci in _sciRepository.Table
                        where sci.UpdatedOnUtc >= olderThanUtc && sci.UpdatedOnUtc <= endDate
                        select sci;
            ...
            int quantity)
        {
            ...
            double ValidQty = (((double)quantity) / ((double)product.IncrementQuantity));
            int numero;

            if(!int.TryParse(ValidQty.ToString(), out numero))
            {
                warnings.Add(string.Format(_localizationService.GetResource("ShoppingCart.QuantityMessage")+" "+ product.IncrementQuantity, product.IncrementQuantity));
                hasQtyWarnings = true;
