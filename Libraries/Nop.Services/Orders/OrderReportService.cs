...
namespace Nop.Services.Orders
{
    ...
    public partial class OrderReportService : IOrderReportService
    {
        ...
        public OrderReportService(CatalogSettings catalogSettings,
            ...
            IRepository<StoreMapping> storeMappingRepository, IRepository<SearchTerm> searchTermRepository)
        {
        ...
        public virtual int[] GetAlsoPurchasedProductsIds(int storeId, int productId,
            int recordsToReturn = 5, int customerId = 0, bool visibleIndividuallyOnly = true, bool showHidden = false)
        {
            ...
            if (productId != 0)
            {
                ...
                var query2 = from orderItem in _orderItemRepository.Table
                             join p in _productRepository.Table on orderItem.ProductId equals p.Id
                            // join o in Order on orderItem.OrderId=
        ...
        #region SearchWords Report
        public virtual IPagedList<SearchTermReportLine> SearchWordsReport(
           DateTime? createdFromUtc = null, DateTime? createdToUtc = null,
           int pageIndex = 0, int pageSize = int.MaxValue)
        {
            
            var query1 = (from st in _searchTermRepository.Table
                          where (!createdFromUtc.HasValue || createdFromUtc.Value.Date <= st.CreatedOnUtc) &&
                          (!createdToUtc.HasValue || createdToUtc.Value.Date >= st.CreatedOnUtc) 
            ...
            var result = new PagedList<SearchTermReportLine>(query1.ToList(), pageIndex, pageSize);
            return result;
        }
