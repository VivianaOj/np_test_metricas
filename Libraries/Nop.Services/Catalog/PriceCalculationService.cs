...
namespace Nop.Services.Catalog
{
    ...
    public partial class PriceCalculationService : IPriceCalculationService
    {
            ...
            out List<DiscountForCaching> appliedDiscounts)
        {
            ...
            if (_workContext.WorkingCompany?.ItemsPricing != null && _workContext.WorkingCompany?.ItemsPricing.FirstOrDefault(s => s.ProductId == product.Id) != null)
            {
                var qty = product.PriceByQtyProduct.Where(s => quantity >= s.Quantity).OrderByDescending(s => s.Quantity);
                var getPricebyCompany = _workContext.WorkingCompany.ItemsPricing.Where(s => s.ProductId == product.Id);
                var pricelevelId = qty.Where(r => getPricebyCompany.Any(p => p.PriceLevel == r.PriceLevelId));

                if (pricelevelId.Count() > 0)
                    price = pricelevelId.Where(s => quantity >= s.Quantity).FirstOrDefault().Price;
                else
                {
                    if (getPricebyCompany.Any(p => p.PriceLevel == -1))
                    {
                        price = getPricebyCompany.FirstOrDefault().Price;
                    }
                    else
                        price = _workContext.WorkingCompany.ItemsPricing.FirstOrDefault(s => s.ProductId == product.Id).Price;
                }
            }
            else
            {
                ...
                if (customer.CustomerRoles.FirstOrDefault(s => s.Id == 4) != null || (customer.NetsuitId == 0 && customer.IsRegistered()))
                {
                    //2= Price Level Retail & Web Store
                    if (product.PriceByQtyProduct.Where(s => s.PriceLevelId == 2 && quantity >= s.Quantity).OrderByDescending(s => s.Quantity).FirstOrDefault() != null)
                        price = product.PriceByQtyProduct.Where(s => s.PriceLevelId == 2 && quantity >= s.Quantity).OrderByDescending(s => s.Quantity).FirstOrDefault().Price;
                    else
                        price = product.Price;
                }
                else
                {
                    if (_workContext.WorkingCompany?.PriceLevelId != null && product.PriceByQtyProduct.Where(s => s.PriceLevelId == _workContext.WorkingCompany?.PriceLevelId && quantity >= s.Quantity).OrderByDescending(s => s.Quantity).FirstOrDefault() != null)
                        price = product.PriceByQtyProduct.Where(s => s.PriceLevelId == _workContext.WorkingCompany?.PriceLevelId && quantity >= s.Quantity).OrderByDescending(s => s.Quantity).FirstOrDefault().Price;

                    else
                    {
                        if (_workContext.WorkingCompany?.PriceLevelId != null && product.PriceByQtyProduct.Where(s => quantity >= s.Quantity).OrderByDescending(s => s.Quantity).FirstOrDefault() != null)
                            price = product.PriceByQtyProduct.Where(s => quantity >= s.Quantity).OrderByDescending(s => s.Quantity).FirstOrDefault().Price;
                        else
                            price = product.Price;
                    }
            ...
            appliedDiscounts.AddRange(result.AppliedDiscounts);
            discountAmount = result.AppliedDiscountAmount;
