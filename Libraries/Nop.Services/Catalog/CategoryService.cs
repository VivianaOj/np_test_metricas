...
namespace Nop.Services.Catalog
{
    ...
    public partial class CategoryService : ICategoryService
    {
        ...
        public virtual IList<Category> GetAllCategoriesDisplayedOnHomepage(bool showHidden = false)
        {
            ...
            var categories = query.OrderBy(r => r.DisplayOrder).ToList();
            if (!showHidden)
            {
        ...
        public virtual Category GetCategoryByName(string name)
        {
            ...
            var query = from p in _categoryRepository.Table
                        where p.Name.Equals(name) && !p.Deleted
                        select p;

            return query.FirstOrDefault();
        }

        public Category GetCategoryByNetsuiteId(int netsuiteId)
        {
            if (netsuiteId == 0)
                return null;

            var query = from p in _categoryRepository.Table
                        where p.NetSuiteId== netsuiteId
                        select p;

            return query.FirstOrDefault();
        }

        public Category GetProductsByCategorynameNetsuite(string netsuiteName, int NetSuiteId, int ParentCategoryId)
        {
            if (netsuiteName == "")
                return null;

            var query =
             from p in _categoryRepository.Table
             where p.Name.ToLower().Trim().Equals(netsuiteName) && p.ParentCategoryId.Equals(ParentCategoryId)
             select p;
            //                from p in _categoryRepository.Table
            //            where p.NetSuiteId.Equals(NetSuiteId)
            //            select p;

            if (query.Count() == 0)
            {
                query = from p in _categoryRepository.Table
                        where p.NetSuiteId.Equals(NetSuiteId)
                        select p;
            }
        ...
        public Category GetProductsByCategorynameNetsuiteValid(string netsuiteName, int NetSuiteId)
        {
            if (netsuiteName == "")
                return null;
            ...
            {
                query = from p in _categoryRepository.Table
                        where p.Name.ToLower().Trim().Equals(netsuiteName)
                        select p;
            }
