...
namespace Nop.Services.Catalog
{
    ...
    public partial class ProductService : IProductService
    {
                    ...
                    bool? overridePublished = null)
        {
            ...
            if (_workContext.CurrentCustomer.Companies.Any())
            {
                if (_workContext.WorkingCompany != null)
                    companyId = Convert.ToInt32(_workContext.WorkingCompany.Id);
            }
            ...
            var pPageIndex = _dataProvider.GetInt32Parameter("PageIndex", pageIndex);
              var pCompanyId = _dataProvider.GetInt32Parameter("CompanyId", companyId);
            ...
            // var products = products2.Where(p => _aclService.AuthorizeItemCollection(p));
			...
			{
                products = products
               .OrderBy(p => p.Name.Contains(keywords)) // Products with name containing keywords first
               .ThenBy(p => p.ProductProductTagMappings.Any(m => m.ProductTag.Name.Contains(keywords))) // Products with tags containing keywords next
               .ThenBy(p => p.ShortDescription.Contains(keywords)) // Products with description containing keywords last
               .ThenBy(p => p.FullDescription.Contains(keywords)) // Products with description containing keywords last
               .ToList();
            }
        ...
        /// <param name="vendorId">Vendor ID; 0 to load all records</param>
        /// <returns>Products</returns>
        public Product GetProductsByNetsuiteItem(string netsuiteId)
        {
            if (netsuiteId == null)
                throw new ArgumentNullException(nameof(netsuiteId));

            var query = _productRepository.Table;
            query = query.Where(p =>/* !p.Deleted &&*/ p.IdInventoryItem== netsuiteId);
        ...
        public int GetProductIdByPictureId(int PictureId)
        {
            int ProducctId = 0;
            var productMapPicture = _productPictureRepository.Table.Where(x => x.PictureId == PictureId).FirstOrDefault();
            if (productMapPicture != null)
            {
                ProducctId = productMapPicture.ProductId;
            }
