...
namespace Nop.Services.Shipping
{
    ...
    public partial class ShippingService : IShippingService
    {
       ...
       // private readonly IPluginManager<IBoxGeneratorServices> _paymentPluginManager;
        ...
        private readonly IBoxesGeneratorServices _boxGeneratorServices;
        ...
        public ShippingService(IAddressService addressService,
            ...
            IBoxesGeneratorServices boxGeneratorServices, ISettingService settingService, IRepository<DeliveryRoutes> repository)
        {
            ...
            _boxGeneratorServices = boxGeneratorServices;
            ...
            foreach (var sci in cart)
            {
                ...
                else
                {
                    ...
                    if (product.ShipSeparately)
                    {
                        ...
                        {
							if (sci.Product.IncrementQuantity > 1)
							{
            ...
            int storeId = 0)
        {
            ...
            var zipPostalCodeFrom = string.Empty;
            var currentWareHouse = GetAllWarehouses().FirstOrDefault();
            if (currentWareHouse != null)
            {
                var currentWareHouseAddress = _addressService.GetAddressById(currentWareHouse.AddressId);
                zipPostalCodeFrom = currentWareHouseAddress != null ? currentWareHouseAddress.ZipPostalCode : string.Empty;
            }
...
//            var shippingRateComputationMethodsBoxGenerator = _shippingPluginManager.LoadActivePlugins(customer, storeId, "Misc.NNBoxGenerator", customer, storeId, storeId);

            //var BoxGenerator = _paymentPluginManager.LoadPluginBySystemName("Misc.NNBoxGenerator", customer, storeId);
            
            //var GetShippingOptions = BoxGenerator.GetBoxesPacking(cart.ToList());

            //shippingOptionRequests.Add(GetShippingOptions);
            ...
            foreach (var srcm in shippingRateComputationMethods)
            {
                ...
                {
                    //clean Box Generator by user
                    _boxGeneratorServices.DeleteBoxGeneratorByUser(customer, cart);
                }

                //request shipping options (separately for each package-request)
                IList<ShippingOption> srcmShippingOptions = null;
                var getShippingOptionResponsePackage = new GetShippingOptionResponse();
                ...
                foreach (var shippingOptionRequest in shippingOptionRequests)
                {
                    ...
                    if (srcm.PluginDescriptor.SystemName == "Shipping.UPS")
                    {
                        foreach (var x in shippingOptionRequest.Items)
                        {
                            var _packageItem = new ShoppingCartItem();
                            if (x.ShoppingCartItem.Product.IncrementQuantity>1)
                            {
                                _packageItem.ProductId = x.ShoppingCartItem.ProductId;
                                _packageItem.Quantity = x.ShoppingCartItem.Quantity;
                                listItems.Add(_packageItem);

                                x.ShoppingCartItem.Quantity = x.ShoppingCartItem.Quantity / x.ShoppingCartItem.Product.IncrementQuantity;

                            }
                        }

                        shippingOptionRequest.ZipPostalCodeFrom = shippingOptionRequest.ZipPostalCodeFrom ?? zipPostalCodeFrom;
                        shippingOptionRequest.Packed = true;

                        var GetPackage = srcm.GetShippingPackingOptions(shippingOptionRequest);

						foreach (var si in listItems)
						{
							var item = shippingOptionRequest.Items.Where(r => r.ShoppingCartItem.ProductId == si.ProductId).FirstOrDefault();
							if (item != null)
								item.ShoppingCartItem.Quantity = si.Quantity;
                    ...
                    else
                    {
                        ...
                        {
                            //Insert Box Generator Information
                            ...
                            if (srcm.PluginDescriptor.SystemName == "Shipping.UPS")
                            {
                                if (isAsShip.Count > 0)
                                {
                                    _boxGeneratorServices.InsertBoxGenerator(getShippingOptionResponse, customer, true, shippingOptionRequest);
                                }
                                else
                                {
                                    if (getShippingOptionResponse.ResultFinal.Count > 0)
                                    {
                                        _boxGeneratorServices.InsertBoxGenerator(getShippingOptionResponse, customer, false, shippingOptionRequest);
                                    }
                                }
                            }
                            //success
                            ...
                            else
                            {
                                ...
                                {
                                    if (srcm.PluginDescriptor.SystemName == "Shipping.NNDelivery")
                                    {
                            ...
                            // Add Freight Quote
                            //srcmShippingOptions.Add(new ShippingOption
                            //{
                            //    Name = _localizationService.GetResource("admin.configuration.shipping.method.freightquote"),
                            //    ShippingRateComputationMethodSystemName = _localizationService.GetResource("admin.configuration.shipping.method.freightquote")
                            //});
                ...
                //shippingOptionRequest.Packed = false;
                //getShippingOptionResponsePackage = srcm.GetShippingOptions(shippingOptionRequest);

                if (srcm.PluginDescriptor.SystemName == "Shipping.UPS")
                {
                    var ShippingOptions = shippingOptionRequests.FirstOrDefault();
                    ShippingOptions.Package = Package;
                    var getShippingOptionResponseOnly = srcm.GetShippingOptions(shippingOptionRequests.FirstOrDefault());

                    if (getShippingOptionResponseOnly.Success)
                    {
                        foreach (var shippingOptionRequest in shippingOptionRequests)
                        {
                            //Insert Box Generator Information
                            ...
                            if (srcm.PluginDescriptor.SystemName == "Shipping.UPS")
                            {
                                if (isAsShip.Count > 0)
                                {
                                    _boxGeneratorServices.InsertBoxGenerator(getShippingOptionResponseOnly, customer, true, shippingOptionRequest);
                                }
                                else
                                {
                                    if (getShippingOptionResponseOnly.ResultFinal.Count > 0)
                                    {
                                        _boxGeneratorServices.InsertBoxGenerator(getShippingOptionResponseOnly, customer, false, shippingOptionRequest);
                                    }
                                }
                            }
                        ...
                        {
                            //first shipping option request
                            srcmShippingOptions = getShippingOptionResponseOnly.ShippingOptions;
                        }
                        else
                        {
                            //get shipping options which already exist for prior requested packages for this scrm (i.e. common options)
                            srcmShippingOptions = srcmShippingOptions
                                .Where(existingso => getShippingOptionResponseOnly.ShippingOptions.Any(newso => newso.Name == existingso.Name))
                                .ToList();

                            //and sum the rates
                            foreach (var existingso in srcmShippingOptions)
                            {
                                existingso.Rate += getShippingOptionResponseOnly
                                    .ShippingOptions
                                    .First(newso => newso.Name == existingso.Name)
                                    .Rate;
                            }
                    ...
                    {
                        //errors
                        foreach (var error in getShippingOptionResponseOnly.Errors)
                        {
                            result.AddError(error);
                            _logger.Warning($"Shipping ({srcm.PluginDescriptor.FriendlyName}). {error}");
                        }
        ...
        public void ValidBoxSelector(Order placedOrder)
        {
            _boxGeneratorServices.SaveBoxGenerator(placedOrder);
        }

        public List<AlgorithmPackingResult> GetBoxByOrder(int id, int customerId)
        {
            return _boxGeneratorServices.GetBoxByOrder(id, customerId);
        }

        public BSBox GetBoxById(int id)
        {
            return _boxGeneratorServices.GetBoxById(id);
        }
        ...
        #region     Start NN Change add location admin order detail. 

        public string GetLocation(Order order)
        {
            string location = "";
            var NetsuiteId = "";
            var PrimaryLocation = "";
            ...
            {
                if (order.Customer.Companies.Count > 0)
                {
            ...
            {
                location = _settingService.GetSetting("ShippingPickupInStore.Nashville").Value;
            }
            if (order.ShippingRateComputationMethodSystemName == "Pickup.PickupInStore")
            {
                ...
                {
                    location = _settingService.GetSetting("ShippingPickupInStore.Atlanta").Value;
                }
                ...
                {
                    location = _settingService.GetSetting("ShippingPickupInStore.Cincinnati").Value;
                }

            }
            var LocationDelivery = "";
            var pickupPoints = GetPickupPoints(order.Customer.BillingAddress, order.Customer, storeId: 1);
            ...
            if (order.ShippingRateComputationMethodSystemName == "Shipping.NNDelivery" || order.ShippingMethod == "Delivery on N&N Truck")
            {
                ...
                if (pickupPoints != null)
                {
                    ...
                    {
                        var pickUpPointNNDelivery = pickupPoints.PickupPoints.Where(r => r.Description == location).FirstOrDefault();

                        LocationDelivery = order.ShippingNNDeliverySend;

                        //[pickUpPointNNDelivery.DeliveryRoute] = LocationDelivery;

                    }
                }

                if (!string.IsNullOrEmpty(LocationDelivery))
                {
                    var getLocation = _repository.Table.FirstOrDefault(r => r.ValueToSend.ToLower() == LocationDelivery.ToLower() && r.Location == location);

                    if (getLocation != null)
                        location = getLocation.Location;
                    else
                    {
                        getLocation = _repository.Table.FirstOrDefault(r => r.ValueToSend.ToLower() == LocationDelivery.ToLower());
                        if (getLocation != null)
                            location = getLocation.Location;
