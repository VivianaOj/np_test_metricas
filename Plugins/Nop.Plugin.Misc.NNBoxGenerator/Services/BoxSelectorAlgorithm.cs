...
{
	public class BoxSelectorAlgorithm : IBoxSelectorAlgorithm
	{
		...
		public AlgorithmPackingResult RunVolumen(Container container, List<Item> items)
		{
			Initialize(container, items);
			ExecuteIterationsTest(container);
			...
			foreach (var item in itemsToPackNN)
			{
                if (item.ID != 0)
                {
					if(!item.IsPacked)
					{
						UnitemsToPack.Add(item);
					}
				}
			}
			result.PackedItems = itemsPackedInOrder;
			result.UnpackedItems = UnitemsToPack;
		...
		private void ExecuteIterationsTest(Container container)
        {
			decimal VolumenPerBox = 0;
			decimal WeightPerBox = 0;

			foreach (var item in itemsToPackNN)
			{
				if (item.ID != 0)
				{
					VolumenPerBox = VolumenPerBox + item.Volume;
					if (VolumenPerBox <= container.Volume)
					{
						WeightPerBox = WeightPerBox + item.WeightPacked;
						if (WeightPerBox <= container.Weight)
						{
							item.IsPacked = true;
							itemsPackedInOrder.Add(item);
							//itemsToPackNN.Add(item);

							SumItemWeightBox = WeightPerBox;
							packedVolume = VolumenPerBox;
							packedItemCount++;
							//itemsToPackCount++;
						}
					}
			...
			//itemsToPackCount = packedItemCount;
		...
		private List<Item> UnitemsToPack;
		private List<Item> itemsToPackNN;
		...
		private decimal totalWeightByBox;
		private decimal totalItemWeight;
		private decimal SumItemWeightBox;
		private decimal packedVolumeWeight;
		...
		private void Initialize(Container container, List<Item> items)
		{
			...
			itemsToPackNN = new List<Item>();
			UnitemsToPack = new List<Item>();
			...
			itemsToPackNN.Add(new Item(0, 0, 0, 0, 0, 0, ""));
			layers = new List<Layer>();
			itemsToPackCount = 0;

			foreach (Item item in items)
			{
					Item newItem = new Item(item.ID, item.Dim1, item.Dim2, item.Dim3, item.WeightPacked, item.Quantity,item.ProductName);
					itemsToPack.Add(newItem);
					itemsToPackNN.Add(newItem);
			...
			totalItemWeight = 0.0M;
			SumItemWeightBox = 0.0M;
			totalWeightByBox = container.Weight;

			for (x = 1; x <= itemsToPackCount; x++)
			{
				totalItemVolume = totalItemVolume + itemsToPack[x].Volume;
				SumItemWeightBox = SumItemWeightBox + itemsToPack[x].WeightPacked;
			}
		...
		{
			var packedVolume2 = packedVolume + itemsToPack[cboxi].Volume;
			var sumActualW = itemsToPack.Where(r => r.IsPacked).Sum(i => i.WeightPacked);

			var SumItemWeightBox2 = sumActualW + itemsToPack[cboxi].WeightPacked;

			if ((packedVolume2 > totalItemVolume) || (SumItemWeightBox2 > totalWeightByBox))
			{
				itemsToPack[cboxi].IsPacked = false;
				//continue;
			}
			else
			{
				SumItemWeightBox = SumItemWeightBox + itemsToPack[cboxi].WeightPacked;
