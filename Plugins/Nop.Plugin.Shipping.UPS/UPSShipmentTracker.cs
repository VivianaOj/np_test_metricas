...
namespace Nop.Plugin.Shipping.UPS
{
    ...
    public class UPSShipmentTracker : IShipmentTracker
    {
        ...
        public virtual IList<ShipmentStatusEvent> GetShipmentEvents(string trackingNumber)
        {
            ...
            var resultShipmentEvent = _upsService.GetShipmentEvents(trackingNumber);
            if (resultShipmentEvent != null && resultShipmentEvent.Any())
            {
                result.AddRange(resultShipmentEvent);
            }

            return result;
        }

        public IList<DeliveryDetail> GetDeliveryDetails(string trackingNumber)
        {
            var results = new List<DeliveryDetail>();

            if (string.IsNullOrEmpty(trackingNumber))
                return results;

            var result = _upsService.GetDeliveryDetails(trackingNumber);
            if (result.Any())
            {
